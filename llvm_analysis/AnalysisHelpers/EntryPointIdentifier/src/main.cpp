...
void _getFuncsFromValue(Value *v, std::set<Function*> &res) {
    if (!v) {
        return;
    }
    ...
    if (dyn_cast<Function>(nv)) {
        ...
        _getFuncsFromValue(si->getTrueValue(), res);
        _getFuncsFromValue(si->getFalseValue(), res);
    } else if (dyn_cast<PHINode>(nv)) {
        PHINode *phi = dyn_cast<PHINode>(nv);
        for (unsigned i = 0; i < phi->getNumIncomingValues(); ++i) {
            Value *iv = phi->getIncomingValue(i);
            _getFuncsFromValue(iv, res);
        }
    }
    return;
}

//Given a function "hfunc", inspect all its invocation sites and extract
//all functions used as a func ptr arg.
int getFuncsFromArg(Function *hfunc, std::set<Function*> &res) {
    if (!hfunc) {
        return 0;
    }
    //Inspect every call site.
    for (Value::user_iterator i = hfunc->user_begin(), e = hfunc->user_end(); i != e; ++i) {
        CallBase *cb = dyn_cast<CallBase>(*i);
        if (!cb) {
            continue;
        }
        //See whether there are any func ptr arg at this call site.
        for (unsigned ai = 0; ai < cb->arg_size(); ++ai) {
            ...
            //The arg should be a function pointer.
            if (!aty->isPointerTy() ||
                !dyn_cast<FunctionType>(aty->getPointerElementType())) {
                continue;
            }
            //A function ptr arg, extract the function.
            _getFuncsFromValue(arg,res);
        }
    }
    return 1;
}
...
int main(int argc, char *argv[]) {
    ...
    //Identify the top-half interrupt handlers, they appear as func ptr arguments of the
    //"request_irq()" family (e.g., request_threaded_irq(), pci_request_irq()).
    std::set<Function*> handlers_t;
    for (Function &func : *m) {
        if (!func.hasName()) {
            continue;
        }
        std::string fn = func.getName().str();
        if (fn.find("request_") == std::string::npos ||
            fn.find("_irq") == std::string::npos) {
                continue;
        }
        //Ok this should be an interrupt handler registration function,
        //we need to parse its arguments to find the actual int handlers.
        getFuncsFromArg(&func,handlers_t);
    }
    for (Function *pf : handlers_t) {
        ffprintf(outputFile, pf, "INT_HANDLER_T");
    }
    /*
    //Identify the bottom-half interrupt handlers, including the tasklets and workqueue
    //functions.
    //For tasklet,
    //https://elixir.bootlin.com/linux/latest/source/include/linux/interrupt.h#L651
    //(1) consider the static/dynamic assignment to "tasklet_struct" struct.
    //(2) match the function prototype:
    //void (*callback)(struct tasklet_struct *t);
    //For workqueue,
    //https://elixir.bootlin.com/linux/latest/source/include/linux/workqueue.h#L97
    //(1) consider the static/dynamic assignment to work_struct
    //(2) match the function prototype:
    //typedef void (*work_func_t)(struct work_struct *work);
    std::set<Function*> handlers_bw, handlers_bt;
    for (Function &func : *m) {
            ...
            stn.find("struct.work_struct.") == 0)
        {
            ...
            handlers_bw.insert(&func);
        } else if (stn == "struct.tasklet_struct" ||
                  stn.find("struct.tasklet_struct.") == 0)
        {
            //bottom-half: tasklet
            handlers_bt.insert(&func);
        }
    }
    for (Function *pf : handlers_bw) {
        ffprintf(outputFile, pf, "INT_HANDLER_BW");
    }
    for (Function *pf : handlers_bt) {
        ffprintf(outputFile, pf, "INT_HANDLER_BT");
    }
    */
