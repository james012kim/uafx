...
//#define DEBUG_INSERT_MOD_INST
    ...
    public:
        ...
        std::string getTypeStr() {
            ...
            return InstructionUtils::getTypeName(this->type);
            /*
            std::string str;
        ...
        Type *getTy() {
            ...
            /*
            if (InstructionUtils::isCompTy(ty) && InstructionUtils::isIndexValid(ty,fieldId)) {
        ...
        bool isTagEquals(TaintTag *dstTag) {
            ...
            if ( (this->priv == dstTag->priv && this->fieldId == dstTag->fieldId) ||
                 (InstructionUtils::isCompTy(this->type) && InstructionUtils::same_types(this->type,dstTag->type) 
        ...
        */
        ...
        //NOTE: we now assume a context can only be created once in CallContext and 
        ...
        void dumpInfo(raw_ostream &OS) {
            ...
            OS << "Type: " << InstructionUtils::getTypeName(this->type) << " | " << this->fieldId << " is_global: " << this->is_global << "\n";
    ...
    public:
        ...
        bool isTaintEquals(const TaintFlag *dstTaint) const {
            ...
            //return (this->instructionTrace == dstTaint->instructionTrace);
            return true;
        }
        ...
        void addInstructionToTrace(InstLoc *toAdd) {
            ...
            if (!this->instructionTrace.empty() && this->instructionTrace.back() == toAdd) {
                return;
            }
    ...
    public:
        ...
        bool addTf(TaintFlag *ntf) {
            ...
            if (loc && loc->hasCtx() && (*loc->ctx->callSites)[0]) {
                this->resetTfs((*loc->ctx->callSites)[0]);
            }
        ...
        void collectWinnerTfs() {
            ...
            callSites.push_back(this->lastReset);
            CallContext *ctx = CallContext::getContext(&callSites);
            assert(ctx);
            for (Instruction *ri : rets) {
                if (!ri) {
                    continue;
                }
                InstLoc *rloc = InstLoc::getLoc(ri, ctx);
                assert(rloc);
        ...
        void resetTfs(Instruction *entry) {
            ...
            for (TaintFlag *tf : this->targetTaint) {
                ...
                if (tf->is_active && loc && loc->getEntryInst() != entry) {
                    tf->is_active = false;
                }
            }
            return;
