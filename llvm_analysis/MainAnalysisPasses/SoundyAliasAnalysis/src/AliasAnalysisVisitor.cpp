...
#define DEBUG_HANDLE_INLINE_POINTER
    ...
    OutsideObject *AliasAnalysisVisitor::createOutsideObj(Value *p, Instruction *I, bool taint) {
        ...
        InstLoc *loc = InstLoc::getLoc((I ? I : p), this->ctx);
        //Ok, now we need to create a dummy object for the pointer "p"..
        OutsideObject *robj = DRCHECKER::createOutsideObj(p,loc);
        if (robj) {
            //Add it to the global pto record, note that this "pto" is an "edge of dummy obj creation".
            this->updatePointsToObjects(p,robj,loc,0,false,true);
            if (!DRCHECKER::sharedObjTyStrs.empty()) {
                //Try to do some bookkeeping if the new object has a type forced by user to be shared cross-entry. 
    ...
    //Just a convenient wrapper for the same named function in GlobalState.
    std::set<PointerPointsTo*> *AliasAnalysisVisitor::getPointsToObjects(Value *srcPointer) {
        return this->currState.getPointsToObjects(this->ctx, srcPointer);
    }
    ...
    bool AliasAnalysisVisitor::isPtoDuplicated(const PointerPointsTo *p0, const PointerPointsTo *p1, bool dbg = false) {
        ...
        if(dbg){
            dbgs() << "Ty0: " << InstructionUtils::getTypeName(o0->targetType) << " Ty1: " << InstructionUtils::getTypeName(o1->targetType) << "\n";
        }
    ...
    int AliasAnalysisVisitor::matchPtoTy(Type *srcTy, PointerPointsTo *pto, Instruction *I, bool create_host) {
        ...
        Type *bty0 = nullptr;
        uint64_t sz0;
        if (InstructionUtils::isSeqTy(srcTy, &bty0, &sz0) && dyn_cast<StructType>(pTy)) {
            Type *bty = nullptr;
            int sz = -1;
        ...
        if (dstfieldId || !InstructionUtils::isCompTy(srcTy)) {
            return 0;
        }
        ...
        if (fd->findTy(obj->targetType) >= 0) {
            ...
            if (!InstructionUtils::isCompTy(obj->targetType)) {
#ifdef DEBUG_UPDATE_POINTSTO
    ...
    void AliasAnalysisVisitor::updatePointsToObjects(Value *p, AliasObject *obj,
                                                     InstLoc *propInst, long dfid,
                                                     bool is_weak, bool is_creation)
    {
        if (!p || !obj) {
            return;
        }
        ...
        pto->is_creation = is_creation;
        ptos.insert(pto);
        return this->updatePointsToObjects(p,&ptos);
    }

    //If "free" is true, delete the useless pto records in "newPointsToInfo",
    //but note that we will not free the container "newPointsToInfo" itself.
    void AliasAnalysisVisitor::updatePointsToObjects(Value *srcPointer,
                            std::set<PointerPointsTo*> *newPointsToInfo, bool free)
    {
        /***
        ...
        std::map<Value*, std::set<PointerPointsTo*>*> *targetPointsToMap = this->currState.getPointsToInfo(this->ctx);
        ...
        << " expected pointee type: " << InstructionUtils::getTypeName(srcPointeeTy) << "\n";
#endif
        ...
        for (PointerPointsTo *currPointsTo : *toAdd) {
            ...
            //Update the pto record on the object side (i.e., a double-linked-list)
            currPointsTo->targetObject->addPointerPointsTo(currPointsTo);
        }
#ifdef DEBUG_UPDATE_POINTSTO
    ...
    bool AliasAnalysisVisitor::hasPointsToObjects(Value *srcPointer) {
        std::set<PointerPointsTo*> *ptos = this->currState.getPointsToObjects(this->ctx, 
                                                                              srcPointer);
    ...
    std::set<PointerPointsTo*> *AliasAnalysisVisitor::makePointsToCopy_emb(Instruction *propInstruction, Value *srcPointer, Value *resPointer,
        ...
        for (PointerPointsTo *currPointsToObj : *srcPointsTo) {
...
#ifdef DEBUG_GET_ELEMENT_PTR
            dbgs() << "AliasAnalysisVisitor::makePointsToCopy_emb(): src pto: " 
            << InstructionUtils::getTypeName(host_type) << " | " << dstField
            << "obj: " << (const void*)hostObj << "\n";
#endif
            if (!hostObj) {
                continue;
            }
            //We must ensure that it points to an embedded composite type in another composite.
            if (!host_type || !InstructionUtils::isCompTy(host_type)) {
#ifdef DEBUG_GET_ELEMENT_PTR
            ...
            if (!ety || !InstructionUtils::isCompTy(ety)) {
#ifdef DEBUG_GET_ELEMENT_PTR
            ...
            if (newObj) {
                PointerPointsTo *newPointsToObj = currPointsToObj->makeCopyP(resPointer,newObj,fieldId,
                                                  InstLoc::getLoc(propInstruction,this->ctx),false);
    ...
    std::set<PointerPointsTo*>* AliasAnalysisVisitor::makePointsToCopy(Instruction *propInstruction, Value *srcPointer,
...
#ifdef DEBUG_GET_ELEMENT_PTR
        dbgs() << "AliasAnalysisVisitor::makePointsToCopy(): #srcPointsTo: "
        << srcPointsTo->size() << " \n";
#endif
        ...
        InstLoc *loc = InstLoc::getLoc(propInstruction,this->ctx);
        for (PointerPointsTo *currPointsToObj : *srcPointsTo) {
            ...
            << InstructionUtils::getTypeName(basePointerType)
            << " cur pto: " << InstructionUtils::getTypeName(pto->targetObject->targetType)
            << " | " << pto->dstfieldId << ", obj: " << (const void*)(pto->targetObject)
            <<  " dst fid: " << fieldId << "\n";
#endif
            ...
            if (r > 0) {
                ...
                if (r == 1 || r == 3) {
                    ...
                    dbgs() << "!!! AliasAnalysisVisitor::makePointsToCopy(): base pointee "
                    << "is a primitive type but we have more than one index...\n";
                    goto fail_next;
                }
...
fail_next:
        ...
        if (newPointsToInfo) {
            for (PointerPointsTo *p : *newPointsToInfo) {
                ...
                << InstructionUtils::getTypeName(p->targetObject->targetType) << " | " << p->dstfieldId
        ...
        if (tmpPointsTo && !tmpPointsTo->empty()) {
            for (PointerPointsTo *currPointsTo : *tmpPointsTo) {
                ...
                if(std::find(targetObjects.begin(), targetObjects.end(), to_check) == targetObjects.end()) {
                    ...
                    currPointsTo->dstfieldId, InstLoc::getLoc(targetInstruction,this->ctx), false);
    ...
    for(auto currPointsToObj : *srcPointsTo) {
        ...
        if(std::find(targetObjects.begin(), targetObjects.end(), to_check) == targetObjects.end()) {
            ...
            PointerPointsTo* pto = currPointsToObj->makeCopyP(srcInstruction, currPointsToObj->targetObject, currPointsToObj->dstfieldId, 
                                                              InstLoc::getLoc(srcInstruction,this->ctx), false);
            toRetPointsTo->insert(toRetPointsTo->begin(), pto);
        }
    }
    if (toRetPointsTo->empty()) {
...
void AliasAnalysisVisitor::visitAllocaInst(AllocaInst &I) {
    ...
    AliasObject *targetObj = new FunctionLocalVariable(InstLoc::getLoc(&I,this->ctx));
#ifdef DEBUG_ALLOCA_INSTR
    dbgs() << "Processed Alloca Instruction, Created new points to information:" << (*newPointsTo) << "\n";
#endif
    this->updatePointsToObjects(&I, targetObj, InstLoc::getLoc(&I,this->ctx));
}
...
void AliasAnalysisVisitor::visitCastInst(CastInst &I) {
    ...
    if (srcPointsToInfo && !srcPointsToInfo->empty()) {
        ...
        for(PointerPointsTo *currPointsToObj : *srcPointsToInfo) {
            ...
            newPointsToObj->addProp(InstLoc::getLoc(&I,this->ctx));
    ...
    void AliasAnalysisVisitor::visitBinaryOperator(BinaryOperator &I) {
        ...
        */
        ...
        if (finalPointsToInfo) {
            delete finalPointsToInfo;
        }
        // Sanity,
        ...
        if (finalPointsToInfo && !finalPointsToInfo->empty()) {
            ...
            if (finalPointsToInfo->size() > 1) {
                ...
                for (PointerPointsTo *pto : *finalPointsToInfo) {
                    pto->loadTag.push_back(new TypeField(InstLoc::getLoc(&I, this->ctx),
                                                         id++, nullptr));
                }
            }
            // Update the points to object of the current instruction.
    ...
    Value *AliasAnalysisVisitor::handleInlinePointerOp(Instruction *I, Value *srcPointer) {
        ...
        //We handle the cases that "srcPointer" is an inline operator,
        //if it's a normal instruction, we should've already processed
        //it and set up the pto records (if any).
        if (dyn_cast<Instruction>(srcPointer) ||
            !dyn_cast<Operator>(srcPointer)) {
            return srcPointer;
        }
        ...
        //Is it an inlined GEP operator? If so, handle it via "visitGetElementPtrOperator()".
    ...
    std::set<PointerPointsTo*> *AliasAnalysisVisitor::getPtos(Instruction *I, Value *srcPointer, bool create_dummy, bool taint) { 
        ...
        //Well, reaching here means that original "srcPointer" doesn't have any pto records, we should try
        //to see whether we have any now (e.g., after handling inline ops) and if so, update the records.
        if (hasPointsToObjects(v)) {
            std::set<PointerPointsTo*> *ptos = this->getPointsToObjects(v);
            if (!hasPointsToObjects(srcPointer)) {
                //We can free neither the container "ptos" nor the pto records inside,
                //because they are already associated with another variable.
                this->updatePointsToObjects(srcPointer,ptos,false);
            }
            //NOTE: we cannot directly return "ptos" because the update for
            //"srcPointer" may discard some ptos for "v" due to type matching.
            return this->getPointsToObjects(srcPointer);
        }
        return nullptr;
    }
        ...
        if (!todo_set->empty()) {
            ...
            for (int i = 2; i < I->getNumOperands(); ++i) {
                ...
                if (i > 2) {
                    ...
                    */
                    //Try an alternative method: simply create the emb obj w/o a pointer value.
                    newPointsToInfo = makePointsToCopy_emb(propInst, nullptr,
                                             (i >= I->getNumOperands()-1 ? I : nullptr),
                                                           todo_set, fid);
                }else {
    ...
    std::set<PointerPointsTo*> *AliasAnalysisVisitor::processGEPFirstDimension(Instruction *propInst, GEPOperator *I) {
        ...
        for (PointerPointsTo *p : *basePointsTo) {
            ...
            np->addProp(InstLoc::getLoc(propInst,this->ctx));
            np->targetPointer = I;
            np->fieldId = 0;
            np->flag = 0;
        ...
        << InstructionUtils::getTypeName(basePointerTy) << "\n";
#endif
    ...
    int AliasAnalysisVisitor::bit2Field(Instruction *propInst, GEPOperator *I, PointerPointsTo *pto, unsigned bitWidth, long index) {
        ...
        while (true) {
...
#endif
            if (!targetObjTy || !InstructionUtils::isCompTy(targetObjTy)) {
#ifdef DEBUG_GET_ELEMENT_PTR
            ...
            tydesc = InstructionUtils::getCompTyDesc(dl,targetObjTy);
            if (!tydesc) {
#ifdef DEBUG_GET_ELEMENT_PTR
            ...
            if (dstfield == -1) {
                ...
                Type *ety = nullptr;
                InstructionUtils::isSeqTy(targetObjTy, &ety);
                assert(ety);
        ...
        if (resOffset < 0 || (uint64_t)resOffset >= dl->getTypeStoreSizeInBits(pto->targetObject->targetType)) {
            ...
            DRCHECKER::createHostObjChain((*tydesc)[i_dstfield],pto,limit,
                                          InstLoc::getLoc(propInst,this->ctx));
    ...
    void AliasAnalysisVisitor::_visitLoadInst(LoadInst &I) {
...
#ifdef CREATE_DUMMY_OBJ_IF_NULL
        std::set<PointerPointsTo*> *srcPointsTo = this->getPtos(&I,srcPointer,true,false);
#else
        ...
        std::set<PointerPointsTo*> *newPointsToInfo = new std::set<PointerPointsTo*>();
        InstLoc *propInst = InstLoc::getLoc(&I,this->ctx);
        long id = 0;
        for (PointerPointsTo *currPointsToObj : *srcPointsTo) {
            ...
            std::set<ObjectPointsTo*> fptos;
            dstObj->fetchPointsToObjects(target_field, fptos, propInst);
            //Record the access (read since it's load) site info
            dstObj->addAccessSite(target_field, propInst, currPointsToObj, true);
            for (ObjectPointsTo *p : fptos) {
                AliasObject *pobj = p->targetObject;
                if (!pobj) {
                    continue;
                }
                long pfid = p->dstfieldId;
                //De-duplication for the pointees.
                auto e = std::make_pair(pfid, pobj);
                if (std::find(finalObjects.begin(), finalObjects.end(), e) != 
                    finalObjects.end()) {
                    continue;
                }
                finalObjects.insert(e);
                //Create the pto for the fetched pointee.
                PointerPointsTo *pto = new PointerPointsTo(&I, dstObj, target_field, pobj, pfid, 
                                                           propInst, false);
                //Set up the load tag to handle the potential N*N update problem.
                pto->loadTag = currPointsToObj->loadTag;
                pto->loadTag.push_back(new TypeField(propInst,
                                       id++,(void*)dstObj,target_field));
                //Track the pto propagation.
                pto->propagatingHistory = p->propagatingHistory;
                pto->add2History(propInst);
                //Done. 
...
#ifdef DEBUG_STORE_INSTR
        dbgs() << "inferSrcDstMap(): loc: ";
        ((InstLoc*)((*stag)[stag->size()-si]->v))->print_light(dbgs(), true);
#endif
    ...
    void AliasAnalysisVisitor::_visitStoreInst(StoreInst &I) {
#ifdef DEBUG_STORE_INSTR
        ...
        for (PointerPointsTo *currPointsTo : *dstPointsTo) {
            ...
            //Record the access (write since it's store) site info
            currPointsTo->targetObject->addAccessSite(currPointsTo->dstfieldId,
                                                      propInst, currPointsTo, false);
        }
        return;
    }
    ...
    void AliasAnalysisVisitor::setupCallContext(CallInst &I, Function *currFunction, CallContext *ctx) {

        std::map<Value*, std::set<PointerPointsTo*>*> *currFuncPointsTo = currState.getPointsToInfo(ctx);
    ...
    //Given a pto and a composite type, return the matched sub- or super- obj of the pointee obj.
    AliasObject *AliasAnalysisVisitor::getObj4Copy(PointerPointsTo *pto, Type *ty, Instruction &I) {
        if (!pto || !InstructionUtils::isCompTy(ty)) {
            return nullptr;
        }
    ...
    PointerPointsTo *AliasAnalysisVisitor::copyObj(Value *dstPointer, PointerPointsTo *srcPto,
                                                   Type *ty, Instruction &propInst,
                                                   bool createHeap) {
        if (!srcPto || !ty) {
            return nullptr;
        }
        InstLoc *loc = InstLoc::getLoc(&propInst,this->ctx);
        PointerPointsTo *np = srcPto->makeCopyP(dstPointer,loc);
        AliasObject *nobj = nullptr;
        if (InstructionUtils::isCompTy(ty)) {
            AliasObject *eobj = this->getObj4Copy(np,ty,propInst);
            if (eobj) {
                AliasObject *pobj = nullptr;
                if (createHeap) {
                    //If required (e.g., when handling memdup(), pre-create a heap obj as the copy
                    //target, instead of using a plain AliasObject).
                    pobj = new HeapLocation(loc, eobj->targetType, nullptr, false);
                }
                nobj = eobj->makeCopy(loc, pobj);
            }else {
#ifdef DEBUG_CALL_INSTR
            ...
            nobj = new HeapLocation(loc, ty, nullptr, false);
    ...
    void AliasAnalysisVisitor::handleMemcpyFunction(std::vector<long> &memcpyArgs, CallInst &I) {
        ...
        bool is_comp = InstructionUtils::isCompTy(ty);
#ifdef DEBUG_CALL_INSTR
        ...
        if (!dstPointsTo || dstPointsTo->empty()) {
            ...
            for (PointerPointsTo *pto : *srcPointsTo) {
                ...
                //Here the src obj is read and the dst obj is written, we need
                //to update the access information for UAF detection.
                pto->targetObject->addAccessSite(-1,loc,pto,true);
                if (np->targetObject) {
                    np->targetObject->addAccessSite(-1,loc,np,false);
                }
            }
            if (!newPtos.empty()) {
                this->updatePointsToObjects(dstOperand,&newPtos);
            }
        } else {
            std::map<PointerPointsTo*,std::set<PointerPointsTo*>> sdMap;
            ...
            for (PointerPointsTo *dpto : *dstPointsTo) {
                ...
                dpto->print(dbgs());
#endif
                //Dst object is written here, mark it for UAF detection
                dpto->targetObject->addAccessSite(-1,loc,dpto,false);
                //First make sure the dst pointee object is capable of receving the copy type..
                AliasObject *dobj = nullptr;
                if (is_comp) {
                    dobj = this->getObj4Copy(dpto,ty,I);
                    if (!dobj) {
#ifdef DEBUG_CALL_INSTR
                ...
                for (PointerPointsTo *spto : *srcs) {
                    ...
                    //Src object is read here, mark it for UAF detection
                    spto->targetObject->addAccessSite(-1,loc,spto,true);
                    AliasObject *sobj = nullptr;
                    if (is_comp) {
                        sobj = this->getObj4Copy(spto,ty,I);
                        if (!sobj) {
#ifdef DEBUG_CALL_INSTR
    ...
    void AliasAnalysisVisitor::handleFdCreationFunction(std::map<long,long> &fdFieldMap, Function *currFunc, CallInst &I) {
        ...
        if (newObj) {
            ...
            for (auto &e : fdFieldMap) {
                ...
                if (arg_no < 0 || arg_no >= (long)I.arg_size()) {
                    continue;
                }
...
#ifdef DEBUG_CALL_INSTR
        dbgs() << "AliasAnalysisVisitor::handleMemdupFunction(): inferred memdup obj type: " << InstructionUtils::getTypeStr(ty) << "\n";
#endif
        //Is this a memdup from the user space? If so, we need to treat it as a user initiated taint source later.
        bool from_user = false;
        //We don't care whether it's form user or not for UAF detection.
        /*
        Function *func = I.getCalledFunction();
        if (func && func->hasName()) {
        ...
        */
        ...
        if (srcPointsTo && !srcPointsTo->empty()) {
            for (PointerPointsTo *pto : *srcPointsTo) {
                ...
                pto->print(dbgs());
#endif
                PointerPointsTo *np = this->copyObj(&I, pto, ty, I, true);
                if (np) {
                    newPtos.insert(np);
                    ...
                    */
                    ...
                    dbgs() << "AliasAnalysisVisitor::handleMemdupFunction(): failed to copy the obj from current src!\n";
                }
            }
        }
        if (!newPtos.empty()) {
            this->updatePointsToObjects(&I,&newPtos);
        }else {
#ifdef DEBUG_CALL_INSTR
            dbgs() << "AliasAnalysisVisitor::handleMemdupFunction(): no pto info copied from src, create new dummy pointee obj!\n";
#endif
            //No src pointees, in this case we need to create the new dummy obj for the dst pointer..
            AliasObject *obj = new HeapLocation(InstLoc::getLoc(&I,this->ctx), ty, sizeArg, false);
    ...
    void AliasAnalysisVisitor::handleCfuFunction(std::set<long> &taintedArgs, CallInst &I) {
        ...
        //so here we only need to take care of the dst pointee object. (i.e. reset the taint and 
        //create a new dummy pointee for the dst pointer if there is not one).
        //NOTE: we don't treat copy_from_user as a taint initiator any more for UAF detection.
...
#endif
                //Create a dummy dst object and set it as a global taint source (since the pointee comes from the thin air).
                //TODO: the "size" arg.
                InstLoc *loc = InstLoc::getLoc(&I,this->ctx);
                AliasObject *obj = new HeapLocation(loc, ty, nullptr, false);                
    ...
    //Mark the specified object as freed at a certain InstLoc.
    void AliasAnalysisVisitor::markFreedObj(AliasObject *obj, Instruction &I, 
                                            PointerPointsTo *pto) {
        if (!obj || !pto) {
            return;
        }
        InstLoc *loc = InstLoc::getLoc(&I,this->ctx);
        obj->markFree(loc, pto);
        //Add the freed obj to the global record.
        this->currState.freedObjs.insert(obj);
    }

    void AliasAnalysisVisitor::markFreedObjs(std::vector<long> &freedArgs, CallInst &I) {
#ifdef DEBUG_CALL_INSTR
        dbgs() << "AliasAnalysisVisitor::markFreedObjs(): Processing free function\n";
#endif
        //Basically, we need to mark all objects pointed to by the freed pointer arg.
        for (long n : freedArgs) {
            Value *p = I.getArgOperand(n);
            if (!p) {
                continue;
            }
            //We must first know the real obj type pointed to by "p", so that we can mark the correct size of the free'd memory region.
            Type *ty = InstructionUtils::inferPointeeTy(p);
            std::set<PointerPointsTo*> *ptos = this->getPointsToObjects(p);
            if (ptos == nullptr || ptos->empty()) {
                //We need to create the dummy obj..
                if (ty) {
                    InstLoc *loc = InstLoc::getLoc(&I, this->ctx);
                    AliasObject *obj = new HeapLocation(loc, ty, nullptr, false);
                    //update the pto for dst pointer.
                    this->updatePointsToObjects(p,obj,loc);
                    ptos = this->getPointsToObjects(p);
                    if (ptos && ptos->size() == 1) {
                        //Successfully create the dummy obj to be freed, mark it. 
                        this->markFreedObj(obj,I,*ptos->begin());
                    }
#ifdef DEBUG_CALL_INSTR
                    dbgs() << "AliasAnalysisVisitor::markFreedObjs(): Create a new dummy dst obj: " << (const void*)obj << " and mark it!\n";
#endif
                }else {
#ifdef DEBUG_CALL_INSTR
                    dbgs() << "!!! AliasAnalysisVisitor::markFreedObjs(): no type inferred for the dummy obj!!\n";
#endif
                }
            }else {
                //ok, mark every pointed obj.
                for (PointerPointsTo *pto : *ptos) {
                    ...
                    AliasObject *obj = pto->targetObject;
                    if (ty) {
                        PointerPointsTo *npto = new PointerPointsTo();
                        npto->targetObject = pto->targetObject;
                        npto->dstfieldId = pto->dstfieldId;
                        int r = this->matchPtoTy(ty,npto,&I,true);
                        if (r == 1 || r == 3) {
                            //TODO: what if the "ty" is not a composite type? That basically means it tries to free a primitive field
                            //within an object, this seems rare..
                            //Get the emb obj.
                            AliasObject *eobj = this->createEmbObj(npto->targetObject,npto->dstfieldId,nullptr,&I);
                            if (eobj) {
                                obj = eobj;
                            }else {
                                //This seems strange...
#ifdef DEBUG_GET_ELEMENT_PTR
                                dbgs() << "!!! AliasAnalysisVisitor::markFreedObjs(): cannot get or create embedded object: "
                                << (const void*)(npto->targetObject) << "|" << npto->dstfieldId << "\n";
#endif
                            }
                        }else if (r > 0) {
                            //The target obj in npto is just what we want.
                            obj = npto->targetObject;
                        }else {
                            //Fail to match.. in this case we conservatively use the original target object.
#ifdef DEBUG_CALL_INSTR
                            dbgs() << "AliasAnalysisVisitor::markFreedObjs(): fail to match the type for the freed mem region!!\n";
#endif
                        }
                    }
#ifdef DEBUG_CALL_INSTR
                    dbgs() << "AliasAnalysisVisitor::markFreedObjs(): mark the pointed obj: " << (const void*)obj << "\n";
#endif
                    //ok, now mark the obj as it's freed. 
                    this->markFreedObj(obj,I,pto);
                }//the for loop to process each free'd pto
            }
        }//the loop to process each free'd arg pointer.
        return;
    }

    VisitorCallback* AliasAnalysisVisitor::_visitCallInst(CallInst &I, Function *currFunc,
            CallContext *oldCtx, CallContext *newCtx) {
#ifdef DEBUG_CALL_INSTR
        ...
        if (currFunc->isDeclaration()) {
            ...
            if (targetChecker->is_memcpy_function(currFunc)) {
            ...
            }else if (targetChecker->is_lock_function(currFunc)) {
#ifdef DEBUG_CALL_INSTR
                dbgs() << "AliasAnalysisVisitor::visitCallInst(): This is a lock/unlock function w/o definition.\n";
#endif
                //Lock/Unlock recording and pairing will be done in the dedicated LocksetAnalysis, here we only analyze the
                //point-to of the lock object, specifically, we handle the inlined GEP operator used as the pointer arg
                //of the lock/unlock functions, figuring out their pto which can then benefit the LocksetAnalysis.
                std::vector<long> lockArgs = targetChecker->get_lock_arguments(currFunc);
                for (long n : lockArgs) {
                    Value *arg = I.getArgOperand(n);
                    if (arg && !hasPointsToObjects(arg)) {
                        if (dyn_cast<GlobalValue>(arg)) {
                            //This should be impossible since global pto has been set up in
                            //the init phase/
                            dbgs() << "!!! AliasAnalysisVisitor::visitCallInst(): no pto for"
                            << " global var: " << InstructionUtils::getValueStr(arg) << "\n";
                            continue;
                        }
                        //Ensure that the pointer arg has pto records so that LocksetAnalysis can utilize it.
                        std::set<PointerPointsTo*> *ptos = this->getPtos(&I,arg,true,true);
                        if (!hasPointsToObjects(arg) && ptos && !ptos->empty()) {
                            //The ptos here should come from the records of the pre-cast value in the global pointsTo map,
                            //so we should not free it, otherwise the pre-cast value's record becomes erased. 
                            std::map<Value*, std::set<PointerPointsTo*>*> *ptoMap = this->currState.getPointsToInfo(this->ctx);
                            (*ptoMap)[arg] = new std::set<PointerPointsTo*>();
                            *(*ptoMap)[arg] = *ptos;
                        }
                    }
                }
            }else if (targetChecker->is_free_function(currFunc)) {
#ifdef DEBUG_CALL_INSTR
                dbgs() << "AliasAnalysisVisitor::visitCallInst(): This is a free function w/o definition.\n";
#endif
                // We want to detect UAF bugs with taint analysis, and the basic idea is to treat freed memory as taint source,
                // which is what we want to do here.
                std::vector<long> freedArgs = targetChecker->get_freed_arguments(currFunc);
                // Set the freed memory region(s) as taint source.
                this->markFreedObjs(freedArgs, I);
                ...
                this->functionHandler->handleFunction(InstLoc::getLoc(&I, this->ctx), currFunc,
        ...
        // Setup call context.
        setupCallContext(I, currFunc, newCtx);

        // Create a AliasAnalysisVisitor
        AliasAnalysisVisitor *vis = new AliasAnalysisVisitor(currState, currFunc, newCtx);
    ...
    VisitorCallback* AliasAnalysisVisitor::visitCallInst(CallInst &I, Function *currFunc,
        ...
        auto t0 = InstructionUtils::getCurTime();
#endif
        VisitorCallback *vis = this->_visitCallInst(I, currFunc, oldCtx, newCtx);
#ifdef TIMING
        dbgs() << "[TIMING] AliasAnalysisVisitor::visitCallInst(): ";
        InstructionUtils::getTimeDuration(t0,&dbgs());
#endif
        return vis;
    }
