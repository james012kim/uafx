...
namespace DRCHECKER {
    ...
    std::map<std::string,std::set<AliasObject*>> sharedObjCache;
    
    std::set<std::string> sharedObjTyStrs;

    Function *currEntryFunc = nullptr;

    unsigned MAX_EQV_OBJ_ENUM_CNT = 0;
    void *curEqvEnumObj = nullptr;
    unsigned curEqvEnumCnt = 0;
    ...
    Type *getLoadedPointeeTy(Instruction *targetInstr) {
        if (targetInstr && dyn_cast<LoadInst>(targetInstr)) {
            ...
            if (ptrTy->isPointerTy()) {
                ...
                if (InstructionUtils::isPrimitiveTy(ty) || !InstructionUtils::isCompTy(ty)) {
    ...
    void AliasObject::resetPtos(long fid, Instruction *entry) {
        ...
        for (ObjectPointsTo *pto : *srcPto) {
            if (pto && pto->propagatingInst) {
                ...
                if (pto->is_active && pil->hasCtx() && (*pil->ctx->callSites)[0] != entry) {
                    //Deactivate it.
    ...
    void AliasObject::getLivePtos(long fid, InstLoc *loc, std::set<ObjectPointsTo*> *retPto) {
...
#ifdef ENABLE_PTO_PATH_COVERAGE_TEST
#ifdef DEBUG_FETCH_POINTS_TO_OBJECTS
        ...
        if (retPto->size() > 0) {
            ...
            for (ObjectPointsTo *pto : *srcPto) {
                if (!pto->propagatingInst || !pto->propagatingInst->hasCtx()) {
                    has_global_pto = true;
                    break;
                }
                InstLoc *pil = pto->propagatingInst;
                if (pil->ctx->callSites->size() == 1 && loc->hasCtx() && 
                    (*loc->ctx->callSites)[0] == (*pil->ctx->callSites)[0] &&
                    pil->inst && pil->inst == (*pil->ctx->callSites)[0]) {
...
#endif
                    if (loc->hasCtx() && (*loc->ctx->callSites)[0]) {
                        CallContext *newCtx = loc->ctx->getPrefix(0);
                        assert(newCtx);
                        InstLoc *il = InstLoc::getLoc((*loc->ctx->callSites)[0], newCtx);
                        assert(il);
    ...
    void AliasObject::getEqvArrayElm(long fid, std::set<TypeField*> &res) {
        ...
        while (obj->parent) {
            ...
            obj = obj->parent;
            if (InstructionUtils::isSeqTy(obj->targetType)) {
                has_seq = true;
            }
        }
        if (!InstructionUtils::isSeqTy(this->targetType) && !has_seq) {
        ...
        for (AliasObject *e : eqs) {
            if (InstructionUtils::isSeqTy(e->targetType)) {
                std::set<long> eqfs;
                if (fid == -1) {
                    eqfs = e->getAllAvailableFields();
                }else {
    ...
    //Get the pto of the "srcfieldId" of this object.
    void AliasObject::fetchPointsToObjects(long srcfieldId, std::set<ObjectPointsTo*> &rptos,
                                           InstLoc *currInst, bool get_eqv, bool create_obj) {
        /***
        ...
        if (currInst && currInst->hasCtx() && (*currInst->ctx->callSites)[0]) {
            host->resetPtos(srcfieldId,(*currInst->ctx->callSites)[0]);
        }
...
#endif
                        ((AliasObject*)e->priv)->fetchPointsToObjects(e->fid,rptos,currInst,false,false);
                    }
                }
            }
            //Free memory
            ...
            for (ObjectPointsTo *obj : livePtos) {
                if (obj->fieldId == srcfieldId && obj->targetObject) {
                    ...
                    if (obj->dstfieldId == 0 && obj->targetObject->isHeapLocation() && 
                        InstructionUtils::isCompTy(expObjTy) && InstructionUtils::isPrimitiveTy(obj->targetObject->targetType)) 
                    {
#ifdef DEBUG_CHANGE_HEAPLOCATIONTYPE
        ...
        //pto for current field who should exist at current top function entry,
        //thus, we need to set the "siteInst" to the entry, instead of the "currInst".
        if (currInst && currInst->hasCtx() && (*currInst->ctx->callSites)[0]) {
            CallContext *newCtx = currInst->ctx->getPrefix(0);
            assert(newCtx);
            InstLoc *il = InstLoc::getLoc((*currInst->ctx->callSites)[0],newCtx);
            assert(il);
            host->createFieldPointee(srcfieldId, rptos, currInst, il);
        } else {
            dbgs() << "!!! AliasObject::fetchPointsToObjects: currInst does not have a valid level 0 entry function!\n";
            host->createFieldPointee(srcfieldId, rptos, currInst);
        }
    }
    ...
    void AliasObject::createFieldPointee(long fid, std::set<ObjectPointsTo*> &rptos,
                                         InstLoc *currInst, InstLoc *siteInst) {
        if (this->is_const) {
#ifdef DEBUG_FETCH_POINTS_TO_OBJECTS
...
#endif
            return hostObj->fetchPointsToObjects(fid,rptos,currInst);
        }
...
#ifdef DEBUG_FETCH_POINTS_TO_OBJECTS
        dbgs() << "AliasObject::createFieldPointee(): about to create dummy obj of type: " << InstructionUtils::getTypeName(real_ty) << "\n"; 
#endif
        ...
        if (real_ty->isFunctionTy()) {
            ...
            for (Function *func : candidateFuncs) {
                ...
                //Update points-to
                ObjectPointsTo *rp = hostObj->addObjectToFieldPointsTo(fid,newObj,siteInst,false);
                if (rp) {
                    rptos.insert(rp);
                }
            }
            ...
            //TODO: should it be "currInst" or "siteInst"?
            newObj->refInst = siteInst;
            ...
            //Update points-to
            ObjectPointsTo *rp = hostObj->addObjectToFieldPointsTo(fid,newObj,siteInst,false,0,true);
            if (rp) {
                rptos.insert(rp);
            }
            ...
            AliasObject *newObj = new HeapLocation(currInst, real_ty, nullptr, false);
#ifdef DEBUG_FETCH_POINTS_TO_OBJECTS
    ...
    AliasObject *AliasObject::getNestedObj(long fid, Type *dty, InstLoc *loc) {
        ...
        while (true) {
            ...
            << " : " << InstructionUtils::getTypeName(ety) << "\n";
#endif
            if (!ety) {
                if (err == 2) {
    ...
    void AliasObject::updateFieldPointsTo(long srcfieldId, std::set<ObjectPointsTo*> *dstPointsTo,
                                          InstLoc *propagatingInstr, int is_weak,
                                          std::set<ObjectPointsTo*> *fptos) {
        /***
         * Add all objects in the provided pointsTo set to be pointed by the provided srcFieldID
         */
#ifdef DEBUG_UPDATE_FIELD_POINT
        dbgs() << "updateFieldPointsTo() for: " << InstructionUtils::getTypeName(this->targetType) << " | " << srcfieldId;
        ...
        //Reactivation check: if there has been a level 0 entry function change (e.g. we have finished analyzing ioctl_0 and started
        //ioctl_1) since last update to the field pto set, we then need a reset.
        if (propagatingInstr && propagatingInstr->hasCtx() &&
            (*propagatingInstr->ctx->callSites)[0]) {
            host->resetPtos(srcfieldId,(*propagatingInstr->ctx->callSites)[0]);
        }
        //Do the dirty work..
        host->updateFieldPointsTo_do(srcfieldId,dstPointsTo,propagatingInstr,is_weak,fptos);
    }

    //Do the real job of field pto update.
    void AliasObject::updateFieldPointsTo_do(long srcfieldId, std::set<ObjectPointsTo*> *dstPointsTo,
                                             InstLoc *propagatingInstr, int is_weak,
                                             std::set<ObjectPointsTo*> *fptos) {
        if (!dstPointsTo || dstPointsTo->empty()) {
            return;
        ...
        for (ObjectPointsTo *pto : *dstPointsTo) {
            ...
            if (unique) {
                ...
                npto->srcObject = this;
                npto->addProp(propagatingInstr);
                npto->is_creation = pto->is_creation;
                //Insert
        ...
        for (ObjectPointsTo *pto : unique_pto) {
            ...
            for (ObjectPointsTo *e : this->pointsTo[srcfieldId]) {
                ...
                if (e->propagatingInst && e->propagatingInst != pto->propagatingInst) {
                    continue;
                }
        ...
        if (fptos) {
            fptos->insert(to_add.begin(), to_add.end());
        }
#ifdef DEBUG_UPDATE_FIELD_POINT
    ...
    void ObjectPointsTo::print(llvm::raw_ostream& OS) {
        if (this->targetObject) {
            OS << "(ID: " << (const void*)this << ") ";
            OS << this->fieldId << " -> " << InstructionUtils::getTypeName(this->targetObject->targetType) << " | " << this->dstfieldId;
            OS << " Tgt Obj ID: " << (const void*)(this->targetObject) << ", c|w|a: "
            << this->is_creation << "|" << this->is_weak << "|" << this->is_active << "\n";
        }
    }

    OutsideObject* createOutsideObj(Type *ty) {
#ifdef DEBUG_CREATE_DUMMY_OBJ_IF_NULL
        dbgs() << "Type-based createOutsideObj(): " << InstructionUtils::getTypeName(ty) << "\n";
#endif
    ...
    OutsideObject* createOutsideObj(Value *p, InstLoc *loc) {
        ...
        if(p){
            dbgs() << InstructionUtils::getValueStr(p) << "  :  " << InstructionUtils::getTypeName(p->getType());
        }
        dbgs() << "\n";
#endif
        ...
        if (dyn_cast<ConstantPointerNull>(p)) {
#ifdef DEBUG_CREATE_DUMMY_OBJ_IF_NULL
            dbgs() << "createOutsideObj(): Null ptr! Cannot create an outside object!\n";
#endif
            return nullptr;
        }
#ifdef DEBUG_CREATE_DUMMY_OBJ_IF_NULL
    ...
    AliasObject *AliasObject::createEmbObj(long fid, Value *v, InstLoc *loc) {
        ...
        Type *ety = this->getFieldTy(fid);
#ifdef DEBUG_CREATE_EMB_OBJ
        dbgs() << "AliasObject::createEmbObj(): host type: "
        << InstructionUtils::getTypeStr(this->targetType) << " | " << fid << " ety: "
        << InstructionUtils::getTypeName(ety) << " ID: " << (const void*)(this) << "\n";
#endif
        if (!InstructionUtils::isCompTy(ety)) {
#ifdef DEBUG_CREATE_EMB_OBJ
            dbgs() << "AliasObject::createEmbObj(): ety is not composite!\n";
#endif
            return nullptr;
        }
        //If a pointer value is specified for the object, we need to match their types.
        if (v && v->getType() && v->getType()->isPointerTy()) {
            Type *expectedPointeeTy = v->getType()->getPointerElementType();
            if (!InstructionUtils::same_types(ety,expectedPointeeTy)) {
#ifdef DEBUG_CREATE_EMB_OBJ
                dbgs() << "AliasObject::createEmbObj(): ety mismatches the expected: "
                << InstructionUtils::getTypeName(expectedPointeeTy);
#endif
                return nullptr;
            }
        }
...
#ifdef DEBUG_CREATE_EMB_OBJ
                dbgs() << "AliasObject::createEmbObj(): emb obj on file: "
                << (const void *)newObj << "\n";
#endif
        }
        if (!newObj || !InstructionUtils::same_types(newObj->targetType,ety)) {
            ...
            if (!newObj) {
                ...
                << InstructionUtils::getTypeName(newObj->targetType) << "\n";
            }
#endif
            if (newObj) {
            ...
            if (newObj) {
#ifdef DEBUG_CREATE_EMB_OBJ
                dbgs() << "AliasObject::createEmbObj(): the emb obj created: "
...
#ifdef DEBUG_CREATE_EMB_OBJ
                dbgs() << "AliasObject::createEmbObj(): Failed to create the emb object!\n";
#endif
            }
        }
        return newObj;
    ...
    AliasObject *AliasObject::createHostObj(Type *hostTy, long field, InstLoc *loc) {
        if (!InstructionUtils::isCompTy(hostTy)) {
#ifdef DEBUG_CREATE_HOST_OBJ
            dbgs() << "AliasObject::createHostObj(): hostTy is not composite!\n";
#endif
            return nullptr;
        }
#ifdef DEBUG_CREATE_HOST_OBJ
        dbgs() << "AliasObject::createHostObj(): curr emb ty: " << InstructionUtils::getTypeName(this->targetType) << "\n";
...
#ifdef DEBUG_CREATE_HOST_OBJ
        dbgs() << "matchFieldName(): try to match the field name: " << n << " of type: " << InstructionUtils::getTypeName(ty) << "\n";
#endif
        if (!ty || !fd || n.size() == 0) {
#ifdef DEBUG_CREATE_HOST_OBJ
    ...
    std::vector<CandStructInf*> *getStructFrom2Fields(DataLayout *dl, Type *ty0, std::string& n0, 
        ...
        //Record the names (no numerical suffix) of the types that we have
        //already considered, in the case there are many identical types with
        ...
        for (StructType *t : tys) {
            ...
            std::string tyn = t->getName().str();
            tyn = InstructionUtils::trim_struct_name(tyn);
            if (processed.find(tyn) != processed.end()) {
                //This type has already been considered.
                continue;
            }
            processed.insert(tyn);
    ...
    CandStructInf *inferContainerTy(Module *m, Value *v, Type *ty, long bitoff) {
        ...
        << InstructionUtils::getTypeName(ty) << " bitoff: " << bitoff << "\n";
#endif
...
#ifdef DEBUG_INFER_CONTAINER
                /*
                dbgs() << "inferContainerTy(): cannot calculate the overall offset for GEP: "
...
#ifdef DEBUG_INFER_CONTAINER
            /*
            dbgs() << "inferContainerTy(): found one GEP using the same srcPointer: "
            << InstructionUtils::getValueStr(gep) << " delta: " << delta << " resOff: "
            << resOff << " current host type size: " << tysz << "\n";
            */
#endif
            ...
            for (Type *t : candTy1) {
                ty1 = t;
                if (InstructionUtils::isCompTy(ty1) || 
                    (ty1->isPointerTy() && !ty1->getPointerElementType()->isIntegerTy())) {
                    break;
                }
            }
            if (!ty1) {
...
#ifdef DEBUG_INFER_CONTAINER
            dbgs() << "inferContainerTy(): ty1: " << InstructionUtils::getTypeName(ty1)
            << " n1: " << n1 << "\n";
#endif
        ...
        for (int i = 0; i < cands.size(); ++i) {
            ...
            dbgs() << "inferContainerTy(): CAND " << i << " SCORE " << cands[i]->score << " : " << InstructionUtils::getTypeName(t) << "\n"; 
    ...
    bool isSharedTy(Type *ty, std::string *s = nullptr) {
        if (DRCHECKER::sharedObjTyStrs.empty() || !ty) {
            return false;
        }
        //Only structure types can be shared.
        ...
        if (DRCHECKER::sharedObjTyStrs.find(n) != DRCHECKER::sharedObjTyStrs.end()) {
            if (s) {
                *s = n;
            }
            return true;
    ...
    int addToSharedObjCache(AliasObject *obj) {
        //Check whether the obj type is what the user specified.
        std::string s;
        if (!obj || !isSharedTy(obj->targetType,&s)) {
            return 0;
        }
#ifdef DEBUG_SHARED_OBJ_CACHE
        dbgs() << "addToSharedObjCache(): obj: " << (const void*)obj << " ty: " << s << "\n";
#endif
        DRCHECKER::sharedObjCache[s].insert(obj);
        return 1;
    }

    int getFromSharedObjCache(Type *ty, std::set<AliasObject*> &res) {
        std::string s;
        if (!ty || !isSharedTy(ty,&s)) {
            return 0;
        }
        if (DRCHECKER::sharedObjCache.find(s) != DRCHECKER::sharedObjCache.end()) {
            res.insert(DRCHECKER::sharedObjCache[s].begin(),DRCHECKER::sharedObjCache[s].end());
            return DRCHECKER::sharedObjCache[s].size();
        }
        return 0;
    }
    ...
    void ObjectPointsTo::printProp(llvm::raw_ostream& OS, bool complete, bool lbreak) {
        bool first = true;
        int cnt = 0;
        for (unsigned i = 0; i < this->propagatingHistory.size(); ++i) {
            InstLoc *loc = this->propagatingHistory[i];
            if (!loc) {
                continue;
            }
            if (first || complete) {
                loc->print_light(OS, false);
                OS << " ~~> ";
            }
            ++cnt;
            first = false;
    ...
    void PointerPointsTo::print(llvm::raw_ostream& OS) {
        ...
        if (this->targetObject) {
            ...
            OS << InstructionUtils::getTypeName(this->targetObject->targetType) << " | " << this->dstfieldId 
        ...
        for (TypeField *tf : this->loadTag) {
            if (tf->v) {
                ((InstLoc *)(tf->v))->print_light(OS, false);
            }
            OS << " (o:" << (const void *)(tf->priv) << ")[" << tf->fid << "] ~~> ";
        }
        // print the propagating history.
        OS << ", prop: ";
        this->printProp(OS, false, false);
        // attributes.
        OS << ", c|w|a: " << this->is_creation << "|" << this->is_weak << "|"
           << this->is_active << "\n";
    }
