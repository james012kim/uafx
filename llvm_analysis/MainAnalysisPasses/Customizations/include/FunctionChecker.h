...
namespace DRCHECKER {
    ...
    public:
        ...
        virtual bool is_free_function(const Function *targetFunction) {
            return false;
        }

        virtual bool is_lock_function(const Function *targetFunction) {
            return false;
        }
        ...
        virtual std::vector<long> get_freed_arguments(const Function *targetFunction) {
            std::vector<long> defaultFreedArgs;
            // make sure that the function is a fd creation.
            assert(this->is_free_function(targetFunction));
            return defaultFreedArgs;
        }

        virtual std::vector<long> get_lock_arguments(const Function *targetFunction) {
            std::vector<long> defaultLockArgs;
            // make sure that the function is a fd creation.
            assert(this->is_lock_function(targetFunction));
            return defaultLockArgs;
        }

        virtual std::set<std::string> get_paired_lock_funcs(const std::string &fn, bool *is_lock) {
            std::set<std::string> pfns;
            return pfns;
        }
    ...
    public:
         ...
         */
        virtual void* handleAllocationFunction(InstLoc *callInstLoc,
                                Function *targetFunction, void *private_data = nullptr) = 0;
        /***
         ...
         * @return opaque pointer whose interpretation depends on the implementation.
         */
        virtual void* handleCustomFunction(InstLoc *callInstLoc,
                                Function *targetFunction, void *private_data = nullptr) = 0;
    ...
    public:
         ...
         */
        void* handleFunction(InstLoc *callInstLoc, Function *targetFunction,
