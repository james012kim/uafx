...
namespace DRCHECKER {
    ...
    std::vector<std::vector<BasicBlock*>*> *BBTraversalHelper::getSCCTraversalOrder(Function &currF) {
        static std::map<const Function*, std::vector<std::vector<BasicBlock*>*>> oCache;
        const Function *F = &currF;
        if (oCache.find(F) != oCache.end()) {
            return &oCache[F];
        }
        ...
        for (scc_iterator<const Function *> I = scc_begin(F), IE = scc_end(F); I != IE; ++I) {
            ...
            oCache[F].insert(oCache[F].begin(), newCurrSCC);
        }
        return &oCache[F];
    }
    ...
    bool bypassCall(std::set<InstLoc*> &bis, unsigned e) {
        ...
        for (InstLoc *bi : bis) {
            if (bi && bi->hasCtx() && e < bi->ctx->callSites->size() && 
                (*bi->ctx->callSites)[e] && (*bi->ctx->callSites)[e-1]) {
                if (!cs) {
                    cs = (*bi->ctx->callSites)[e-1];
                }else if (cs != (*bi->ctx->callSites)[e-1]) {
                    //This doesn't match our pre-condition about this function - blcokers should originate from the same call site.
                    continue;
                }
                callBis[(*bi->ctx->callSites)[e]].insert(bi);
            }
        }
        if (callBis.empty()) {
            return true;
        ...
        for (auto &p : callBis) {
            ...
            for (InstLoc *bi : ebis) {
                if (e + 1 >= bi->ctx->callSites->size()) {
                    ...
                    //Some sanity checks..
                    Instruction *I0 = (*bi->ctx->callSites)[e];
                    Instruction *I1 = (*bi->ctx->callSites)[e+1];
    ...
    void InstLoc::getBlockersInCurrFunc(std::set<InstLoc*> *blocklist, std::set<Instruction*> &validBis) {
        ...
        for (InstLoc *bi : *blocklist) {
            if (bi && bi->hasCtx()) {
                ...
                if (r == 0) {
                    ...
                    if ((*bi->ctx->callSites)[r]) {
                        callsiteBis[(*bi->ctx->callSites)[r]].insert(bi);
                    }
                }
            }
        }
        ...
        for (auto &e : callsiteBis) {
            ...
            if (!bypassCall(bis,this->ctx->callSites->size()+1)) {
                validBis.insert(cs);
            }
        }
        return;
    ...
    std::map<Instruction*,std::map<Instruction*,std::set<CallContext*>>> CallContext::dir;

    // Get an existing context, or create a new one if required.
    CallContext *CallContext::getContext(std::vector<Instruction *> *callSites,
        ...
        if (callSites->size() % 2 == 0) {
            // A valid calling context must have an odd length, according to
            // the sequence format.
            dbgs() << "!!! getContext(): Invalid calling context length\n";
            return nullptr;
        }
        // First search existing contexts.
        std::set<CallContext *> &ctxs =
            dir[callSites->at(0)][callSites->at(callSites->size() - 1)];
        for (CallContext *ctx : ctxs) {
            if (ctx->same(callSites)) {
                return ctx;
            }
        }
        // No matching caontext on file, create one if required.
        if (!create) {
            return nullptr;
        }
        if (created) {
        ...
        CallContext *newCtx = new CallContext(callSites);
        ctxs.insert(newCtx);
        return newCtx;
    }

    std::map<Value*,std::set<InstLoc*>> InstLoc::dir;

    InstLoc *InstLoc::getLoc(Value *inst, CallContext *ctx, bool create) {
        // For now we allow a null "ctx", which may help to represent a global
        // location w/o a calling context.
        if (!inst) {
            return nullptr;
        }
        // First search existing InstLocs.
        std::set<InstLoc *> &locs = dir[inst];
        for (InstLoc *loc : locs) {
            if (loc->ctx == ctx) {
                return loc;
            }
        }
        // No matching InstLoc on file, create one if required.
        if (!create) {
            return nullptr;
        }
        InstLoc *newLoc = new InstLoc(inst, ctx);
        locs.insert(newLoc);
        return newLoc;
    }
    ...
    void BBTraversalHelper::getUseBBs(Value *v, std::set<llvm::BasicBlock*> &res) {
        if (!v) {
            return;
        }
        for (User *u : v->users()) {
            ...
            if (dyn_cast<Instruction>(u)) {
                ...
                if (inst->getParent()) {
                    res.insert(inst->getParent());
                }
            } else if (dyn_cast<Operator>(u)) {
                //"v" might be used in an embedded operator, in which case
                //we need to recursively find the inst user of the operator.
                BBTraversalHelper::getUseBBs(dyn_cast<Value>(u),res);
            }
        }
        //Not sure whether the user list includes "v" itself if it's an inst...
    ...
    void BBTraversalHelper::getRetBBs(llvm::Function* pfunc, std::set<llvm::BasicBlock*> &r) {
        ...
        static std::map<Function*,std::set<BasicBlock*>> cache;
        if (cache.find(pfunc) != cache.end()) {
            goto out;
        }
        for (llvm::BasicBlock &bb : *pfunc) {
            if (getSuccNum(&bb) == 0) {
                cache[pfunc].insert(&bb);
            }
        }
out:
        r = cache[pfunc];
    ...
    //Similar to "instPostDom".
    bool BBTraversalHelper::instDom(Instruction *src, Instruction *end, bool is_strict) {
        if (!end || !src || end->getFunction() != src->getFunction()) {
            return false;
        }
        BasicBlock *bsrc = src->getParent();
        ...
        if (bsrc != bend) {
            Function *func = end->getFunction();
            llvm::DominatorTree *domT = BBTraversalHelper::getDomTree(func);
            if (!domT) {
                return false;
            }
            return domT->dominates(bsrc,bend);
        }
        //Ok they are within the same BB, we simply look at their relative order.
        if (src == end) {
            return !is_strict;
        ...
        BasicBlock::const_iterator I = bsrc->begin();
        for (; &*I != src && &*I != end; ++I);
        //If src comes before end in a same BB, then src dominates end.
        return &*I == src;
    }

    //See the func doc in the header file.
    std::map<BasicBlock *, std::set<BasicBlock *>> *BBTraversalHelper::getCondCoverMap(
        BasicBlock *bb) {
        static std::map<BasicBlock*,std::map<BasicBlock*, std::set<BasicBlock*>>> cache;
        if (!bb) {
            return nullptr;
        }
        Instruction *tail = bb->getTerminator();
        if (!tail || tail->getNumSuccessors() <= 1) {
            //Only multi-successor BBs are in scope.
            return nullptr;
        }
        ...
        //Build the cover map now.
        //First collect the direct successors.
        std::set<BasicBlock*> succs;
        for (unsigned i = 0; i < tail->getNumSuccessors(); ++i) {
            BasicBlock *succ = tail->getSuccessor(i);
            if (succ) {
                succs.insert(succ);
            }
        }
...
#endif
        // In the cover map we only include those direct succ BBs
        // that must be governed by "bb", meeting two requirements:
        //(1) it's dominated by "bb" (otherwise there is a way to
        // reach it completely evasing "bb").
        //(2) there exists at least one other direct succ BB that
        // cannot reach it (otherwise every branch of "bb" can reach
        // it, meaning that the conditional of "bb" has no restrictions
        // for it).
        std::set<BasicBlock*> doms;
        BBTraversalHelper::getDominatees(bb,doms,false);
        for (BasicBlock *succ : succs) {
            if (succ == bb || doms.find(succ) == doms.end()) {
                //In these cases, "succ" is not constrained by
                //"bb" conditional.
                continue;
            }
            //Now see whether this "succ" can be reached from other
            //different ones, if so, by which?
            std::set<BasicBlock*> r_succs{succ};
            std::set<Instruction*> blks{bb->getFirstNonPHIOrDbg()};
            for (BasicBlock *o_succ : succs) {
                if (o_succ == succ || o_succ == bb) {
                    //It's impossible that the constraint on the self-edge
                    //to "bb" can apply to "succ"...
                    continue;
                }
                //Have loop in mind: we will test whether "o_succ" can
                //reach "succ", but not via a loop back, where "bb"
                //should be on the way.
                if (isPotentiallyReachable(o_succ->getFirstNonPHIOrDbg(),
                    succ->getFirstNonPHIOrDbg(),0,&blks)) {
                    r_succs.insert(o_succ);
                }
            }
            if (r_succs.size() < succs.size()) {
                //This means not every direct succ of "bb" can reach "succ",
                //in other words, "bb" conditional has some effects on "succ".
                //NOTE: one implication is that if "bb" has a self-edge, all
                //other direct succs will be constrained that "only when the
                //condition of self-edge is false, they can be reached".
                cache[bb][succ] = r_succs;
            }
        }
        //For debug..
        ///////////////////////////////////////////
        /*
        dbgs() << "BBTraversalHelper::getCondCoverMap(): for: "
        << InstructionUtils::getValueStr(tail) << "\n";
        for (auto &e : cache[bb]) {
            dbgs() << InstructionUtils::getBBStrID(e.first) << " : ";
            for (BasicBlock *bb : e.second) {
                dbgs() << InstructionUtils::getBBStrID(bb) << ", ";
            }
            dbgs() << "\n";
        }
        */
        ///////////////////////////////////////////
#ifdef TIMING
        dbgs() << "[TIMING] BBTraversalHelper::getCondCoverMap: ";
        InstructionUtils::getTimeDuration(t0,&dbgs());
#endif
        return &cache[bb];
    }

    unsigned _succ2No(Instruction *ti, BasicBlock *bb) {
        unsigned i = 0;
        bool matched = false;
        for (; i < ti->getNumSuccessors(); ++i) {
        ...
        assert(matched);
        if (!matched) {
            dbgs() << "!!! _succ2No: no succ match for tail: "
                   << InstructionUtils::getValueStr(ti) << ", and succ: "
    ...
    std::map<llvm::Instruction*, unsigned> *BBTraversalHelper::getCriticalBranches(
                                                        llvm::Instruction *inst) {
        if (!inst || !inst->getParent()) {
            return nullptr;
        }
        //dbgs() << "getCriticalBranches for " << InstructionUtils::getValueStr(inst) << "\n";
        //A cache for a better performance.
        static std::map<BasicBlock*,std::map<llvm::Instruction*, unsigned>> cache;
        BasicBlock *bb = inst->getParent();
        if (cache.find(bb) != cache.end()) {
            return &(cache[bb]);
        }
        //Do the work, first get all dominators of "bb" as candidates, because
        //a critical branch inst of "inst" must also dominate it.
        std::set<BasicBlock*> doms;
        BBTraversalHelper::getDominators(bb,doms,false);
        //dbgs() << "#doms: " << doms.size() << "\n";
        //In order to be a critical branch of "inst", the candidate must:
        //(1) have more than one successors, and
        //(2) one of its successors must dominate "inst".
        //Now get the final critical branches based on above principles.
        for (BasicBlock *dom : doms) {
            if (!dom) {
                continue;
            }
            std::map<BasicBlock*, std::set<BasicBlock *>> *cMap = 
                                        BBTraversalHelper::getCondCoverMap(dom);
            if (!cMap || cMap->empty()) {
                //No restrictions posed by this cond BB to its successors.
                continue;
            }
            Instruction *tail = dom->getTerminator();
            for (auto &e : *cMap) {
                if (e.second.size() > 1) {
                    //This cond BB is likely a switch, while current
                    //succ can be reached in multiple cases, in other
                    //words, we will have to return multiple case
                    //"dst" numbers that can potentially reach "inst",
                    //but now we only consider the situation where
                    //"inst" can only be reached in *one* branch, w/
                    //one "dst" number returned, so skip here.
                    //TODO: support multiple "dst" number handling.
                    continue;
                }
                if (e.first == bb || doms.find(e.first) != doms.end()) {
                    cache[bb][tail] = _succ2No(tail,e.first);
                    break;
                }
            }
        }
    ...
    void InstLoc::getCriticalBranches(std::map<InstLoc*,unsigned> &res) {
        //Basically, we will consider the critical branches not only for this->inst,
        //but also for all the call sites in the context, this way we can get the
        //critical bracnhes inter-procedurally.
        res.clear();
        Instruction *inst = dyn_cast<Instruction>(this->inst);
        CallContext *cctx = this->ctx;
        //dbgs() << "getCriticalBranches() for: " << InstructionUtils::getValueStr(inst) << "\n";
        while (inst) {
            std::map<llvm::Instruction*, unsigned> *cbs = 
                                BBTraversalHelper::getCriticalBranches(inst);
            if (cbs) {
                for (auto &e : *cbs) {
                    InstLoc *cloc = InstLoc::getLoc(e.first,cctx);
                    if (cloc) {
                        //dbgs() << "getCriticalBranches(): find one critical branch: ";
                        //cloc->print_light(dbgs(), true);
                        res[cloc] = e.second;
                    }
                }
            }
            //Go upward to the last call site.
            if (!cctx || !cctx->callSites || cctx->callSites->size() < 3) {
                break;
            }
            inst = cctx->callSites->at(cctx->callSites->size() - 2);
            cctx = cctx->getParentContext();
            //dbgs() << "inst: " << InstructionUtils::getValueStr(inst)
            //<< ", #ctx: " << cctx->callSites->size() << " " << 
            //(const void*)cctx << "\n";
        }
        return;
    }
    ...
    void InstLoc::print(raw_ostream &O) {
        if (this->inst) {
            ...
            if (this->ctx && this->ctx->callSites->size() > 0) {
                O << "[";
                std::string lastFunc;
                for (Instruction *inst : *this->ctx->callSites) {
    ...
    void InstLoc::print_light(raw_ostream &O, bool lbreak) {
        if (this->inst) {
            ...
            O << " (";
            if (this->hasCtx()) {
                InstructionUtils::printCallingCtx(O,this->ctx->callSites,false);
            }
            O << ")";
        }
        if (lbreak) {
    ...
    bool InstLoc::postDom(InstLoc *other, bool is_strict) {
        ...
        if (other->hasCtx()) {
            ...
            assert(this->ctx->callSites->size() % 2);
            assert(other->ctx->callSites->size() % 2);
            while (ip < this->ctx->callSites->size() && 
                   ip < other->ctx->callSites->size() && 
                   (*this->ctx->callSites)[ip] == (*other->ctx->callSites)[ip] &&
                   ++ip);
            if (ip == 0) {
            ...
            if (ip >= this->ctx->callSites->size()) {
                ...
                if (ip < other->ctx->callSites->size()) {
                    src = (*other->ctx->callSites)[ip];
                }
                if (!end || !src || end->getFunction() != src->getFunction()) {
                    //Is this possible?
            ...
            if (ip >= other->ctx->callSites->size()) {
                ...
                Instruction *src = dyn_cast<Instruction>(other->inst);
                Instruction *end = dyn_cast<Instruction>((*this->ctx->callSites)[ip]);
                if (!end || !src || end->getFunction() != src->getFunction()) {
                    //Is this possible?
#ifdef DEBUG_INTER_PROC_POSTDOM
                ...
                if (!BBTraversalHelper::instPostDom((*other->ctx->callSites)[ip],
                                                    (*this->ctx->callSites)[ip])) {
                    return false;
                }
            }
            ++ip;
        ...
        assert(!(ip % 2));
        assert(ip < this->ctx->callSites->size());
        while (ip + 1 < this->ctx->callSites->size()) {
            if (!BBTraversalHelper::instPostDom((*this->ctx->callSites)[ip],
                                                (*this->ctx->callSites)[ip+1])) {
                return false;
            }
            ++ip;
        }
        Instruction *end = dyn_cast<Instruction>(this->inst);
        if (end && !BBTraversalHelper::instPostDom((*this->ctx->callSites)[ip],end)) {
            return false;
        }
        return true;
    }

    //The basic logic is similar to "postDom", but with a different direction.
    bool InstLoc::dom(InstLoc *other, bool is_strict) {
        if (!other) {
            return false;
        }
        if (!this->hasCtx() || !other->hasCtx()) {
            return false;
        }
        int ip = 0;
        assert(this->ctx->callSites->size() % 2);
        ...
        while (ip < this->ctx->callSites->size() &&
               ...
               ++ip);
        if (ip == 0) {
            //Both have calling contexts but no common prefix... This means the top-level entry functions are different, no way to dominate.
            return false;
        }
        //The two calling contexts diverges at a certain point, here we have different situations:
        //1. They diverge within a same caller.
        ...
        //1.3. "this" takes callee A while "other" is a normal inst
        //1.4. both are normal inst
        //For 1. we need to first see whether "this" dominates "other" within the common caller, if not return false immediately, otherwise
        //we need to further inspect the remaining part of "this" calling context to see whether it *must* reach this->inst.
        //2. They diverge at a same call site and take different callees (e.g. an indirect call), in this case "this" cannot dominate "other".
        if (ip >= this->ctx->callSites->size()) {
            //This must be case 1.2. or 1.4., since there are no further context for "this", no need for further inspect.
            Instruction *src = dyn_cast<Instruction>(this->inst);
            Instruction *end = dyn_cast<Instruction>(other->inst);
            if (ip < other->ctx->callSites->size()) {
                end = (*other->ctx->callSites)[ip];
            }
            if (!end || !src || end->getFunction() != src->getFunction()) {
                //Is this possible?
#ifdef DEBUG_INTER_PROC_DOM
                dbgs() << "InstLoc::dom(): !end || !src || end->getFunction() != src->getFunction() - 0\n"; 
#endif
                return false;
            }
            //NOTE: the "is_strict" only makes sense when two InstLoc are exactly the same, if that happens here is the only point we can reach
            //and need to honor the "is_strict" flag in "instDom".
            return BBTraversalHelper::instDom(src,end,is_strict);
        }
        if (ip >= other->ctx->callSites->size()) {
            //This must be case 1.3. We may still need to inspect the remaining "this" context.
            Instruction *end = dyn_cast<Instruction>(other->inst);
            Instruction *src = dyn_cast<Instruction>((*this->ctx->callSites)[ip]);
            if (!end || !src || end->getFunction() != src->getFunction()) {
                //Is this possible?
#ifdef DEBUG_INTER_PROC_DOM
                dbgs() << "InstLoc::dom(): !end || !src || end->getFunction() != src->getFunction() - 1\n"; 
#endif
                return false;
            }
            if (!BBTraversalHelper::instDom(src,end)) {
                return false;
            }
        }else {
            //Ok, the common prefix are well contained in both contexts, it may be case 1.1. or 2., we can differentiate them by index oddness. 
            ...
            //case 1.1., we may still need further inspect.
            if (!BBTraversalHelper::instDom((*this->ctx->callSites)[ip],
                                            (*other->ctx->callSites)[ip])) {
                return false;
            }
        }
        ++ip;
        //(2) Then we need to decide the dominance relationship between the caller entrance and callee invocation site of each function
        //in the remaining calling context (beside the common prefix) of "this".
        //NOTE: at this point "ip" must be the index of an entry inst in "this" calling context.
        assert(!(ip % 2));
        assert(ip < this->ctx->callSites->size());
    ...
    bool InstLoc::chainable(int ci, std::set<InstLoc*> *blocklist, bool callFrom) {
        ...
        if (callFrom) {
            ...
            for (;ci < this->ctx->callSites->size(); ci += 2) {
                Instruction *I = (*this->ctx->callSites)[ci];
                if (I && I->getParent()) {
                    CallContext *newCtx = this->ctx->getPrefix(ci - 1, true);
                    assert(newCtx);
                    InstLoc *il = InstLoc::getLoc(I, newCtx);
                    assert(il);
            ...
            for (int i = this->ctx->callSites->size() - 2; i >= ci; i -= 2) {
                Instruction *I = (*this->ctx->callSites)[i];
                if (I && I->getParent()) {
                    CallContext *newCtx = this->ctx->getPrefix(i - 1, true);
                    assert(newCtx);
                    InstLoc *il = InstLoc::getLoc(I, newCtx);
                    assert(il);
    ...
    bool InstLoc::reachable(InstLoc *other, std::set<InstLoc*> *blocklist) {
        ...
        if (ip >= this->ctx->callSites->size()) {
            ...
            if (blocklist && !blocklist->empty() &&
                ip < other->ctx->callSites->size() &&
                !other->chainable(ip,blocklist,false)) {
        ...
        }else if (ip >= other->ctx->callSites->size()) {
            //Case 1.3.
        ...
        std::set<Instruction*> validBis;
        if (blocklist && !blocklist->empty()) {
            CallContext *newCtx = this->ctx;
            if (ip < this->ctx->callSites->size()) {
                newCtx = this->ctx->getPrefix(ip - 1, true);
            }
            assert(newCtx);
            InstLoc *il = InstLoc::getLoc(src, newCtx);
            assert(il);
    ...
    //Decide the relative position between "this" and "refloc".
    //refloc: a reference inst location, used in combination with "pos".
    //pos: -1: this should reach "refloc";
    //     1: "refloc" should reach "this";
    //     0: either way, but "this" and "refloc" cannot be mutually unreachable.
    //Return whether the positioning requirements are satisfied.
    bool InstLoc::mreachable(InstLoc *refloc, int pos) {
        if (!refloc) {
            return false;
        }
        if (!refloc->ctx || refloc->ctx->empty()) {
            //"refloc" should be a global position w/o any ctx.
            //In this case we assume all positioning are ok.
            return true;
        }
        if (pos < 0) {
            return refloc->reachable(this);
        } else if (pos > 0) {
            return this->reachable(refloc);
        }
        return (refloc->reachable(this) || this->reachable(refloc));
    }
    ...
    void printInstlocJson(InstLoc *inst, llvm::raw_ostream &O) {
        ...
        if (inst->hasCtx()) {
            O << ",\"ctx\":[";
            bool comma = false;
            for (Instruction *ci : *inst->ctx->callSites) {
                if (ci) {
                    if (comma) {
                        O << ",";
                    }
    ...
    void getCtxOfLocTr(const std::vector<InstLoc*> *tr, std::vector<CallContext*> &res) {
        if (!tr) {
            return;
        }
        CallContext *last = nullptr;
        for (InstLoc *il : *tr) {
            if (!il || !il->ctx) {
                continue;
            }
    ...
    //Given an value with the calling context (wrapped in "loc"), locate the load insts
    //- if any - that loads it from mem. The search is inter-procedure.
    int getSrcLoad(InstLoc *loc, std::set<InstLoc*> &res) {
        if (!loc) {
            return 0;
        }
        Value *np = InstructionUtils::stripAllCasts(loc->inst, false);
        if (!np) {
            return 0;
        }
        if (dyn_cast<LoadInst>(np)) {
            res.insert(InstLoc::getLoc(np, loc->ctx, true));
            return 1;
        }
        //Inter-procedure tracking if it's the case.
        if (dyn_cast<Argument>(np)) {
            unsigned argIdx = dyn_cast<Argument>(np)->getArgNo();
            if (loc->hasCtx() && loc->ctx->callSites->size() > 2) {
                CallInst *callsite = dyn_cast<CallInst>((*loc->ctx->callSites)[loc->ctx->callSites->size()-2]);
                if (!callsite || callsite->arg_size() < argIdx + 1) {
                    return 0;
                }
                Value *arg = callsite->getArgOperand(argIdx);
                if (!arg) {
                    return 0;
                }
                //Obtain the InstLoc for the actual argument.
                CallContext *newCtx = loc->ctx->getPrefix(loc->ctx->callSites->size() - 3, true);
                assert(newCtx);
                InstLoc *newLoc = InstLoc::getLoc(arg, newCtx, true);
                assert(newLoc);
                return getSrcLoad(newLoc, res);
            }
            //Otherwise this should be the arg of the top-level entry, we're unable
            //to find the load.
            return 0;
        } else if (dyn_cast<CallInst>(np)) {
            //The thread id is returned by a callee.
            //TODO: handle this case when encountered.
            return 0;
        }
        return 0;
    }
    
    //Return true if "p" eventually points to local memory (e.g., a stack based obj),
    //and "p" is obtained though top-level variable transformation but no mem loads.
    //This function can do the inter-procedure tracking.
    //Return: 1 if it's local, negative values if not, where:
    //-1: originates from an arg of a top entry.
    //-2: others.
    int is_local_ptr(InstLoc *loc) {
        if (!loc) {
            return -2;
        }
        Value *np = InstructionUtils::stripAllSoleTrans(loc->inst);
        if (!np) {
            return -2;
        }
        if (dyn_cast<AllocaInst>(np)) {
            return 1;
        }
        //See whether we need to do the inter-procedure analysis.
        if (dyn_cast<Argument>(np)) {
            //"np" is a function arg, recursively track its origin from the call site.
            unsigned argIdx = dyn_cast<Argument>(np)->getArgNo();
            if (loc->hasCtx() && loc->ctx->callSites->size() > 2) {
                CallInst *callsite = dyn_cast<CallInst>((*loc->ctx->callSites)[loc->ctx->callSites->size()-2]);
                if (!callsite || callsite->arg_size() < argIdx + 1) {
                ...
                CallContext *newCtx = loc->ctx->getPrefix(loc->ctx->callSites->size() - 3, true);
                assert(newCtx);
                InstLoc *newLoc = InstLoc::getLoc(arg, newCtx);
                assert(newLoc);
                return is_local_ptr(newLoc);
            }
            //Otherwise this should be the arg of the top-level entry, we thus 
            //assume it's not from the local memory.
            return -1;
        } else if (dyn_cast<CallInst>(np)) {
            //"np" is a ret value of a callee, the assumption here is that it's less likely
            //that a function returns a pointer to one of its local objects cause they will
            //be invalidated after the function returns.
            return -2;
        }
        //To be conservative.
        return -2;
    ...
    bool is_loc_a2u(InstLoc *aloc, InstLoc *uloc) {
        if (!aloc || !uloc || !aloc->inst || !uloc->inst) {
            return false;
        }
        if (aloc == uloc) {
            //This means we have successfully traced the U site pointer
            //locally to the A site. 
            return true;
        }
        Value *np = InstructionUtils::stripAllSoleTrans(uloc->inst);
        if (!np) {
            //This seems impossible, but anyway return false if so.
            return false;
        }
        //TODO: need also to consider the calling context.
        if (np == aloc->inst) {
            //This means we have successfully traced the U site pointer
            //locally to the A site. 
            return true;
        }
        //So what?  We haven't reached the A site but the strip is already impossible.
        //Is this an inter-procedure case?
        if (dyn_cast<Argument>(np)) {
            //"p" comes from a function arg that might be derived from the A site in the caller.
            //Get the InstLoc for its corresponding actual argument, and recursively search.
            unsigned argIdx = dyn_cast<Argument>(np)->getArgNo();
            if (uloc->hasCtx() && uloc->ctx->callSites->size() > 2) {
                CallInst *callsite = dyn_cast<CallInst>((*uloc->ctx->callSites)[uloc->ctx->callSites->size()-2]);
                if (!callsite || callsite->arg_size() < argIdx + 1) {
                    return false;
                }
                Value *arg = callsite->getArgOperand(argIdx);
                ...
                //Obtain the InstLoc for the actual argument.
                CallContext *newCtx = uloc->ctx->getPrefix(uloc->ctx->callSites->size() - 3, true);
                assert(newCtx);
                InstLoc *newLoc = InstLoc::getLoc(arg, newCtx);
                assert(newLoc);
                return is_loc_a2u(aloc,newLoc);
            }
        } else if (dyn_cast<CallInst>(np)) {
            //"p" is the return value of a call, maybe A site is just in that call and make its return as also that call's return.
            //Here we only handle the cases where the call has a unique return value that we can continue tracing upwards.
            Function *callee = dyn_cast<CallInst>(np)->getCalledFunction();
            if (!callee || callee->isDeclaration()) {
                return false;
            }
            std::set<Instruction*> rets;
            BBTraversalHelper::getRetInsts(callee, rets);
            //Try to get the unique return value.
            Value *rv = nullptr;
            for (Instruction *ri : rets) {
                //Each ret must return the single same variable.
                if (!ri) {
                    continue;
                }
                ...
                if (!v || dyn_cast<Constant>(v)) {
                    return false;
                }
                if (!rv) {
                    rv = v;
                } else if (rv != v) {
                    //No unique return value, cannot continue tracing.
                    return false;
                }
            }
            if (!rv) {
            ...
            //Ok, continue tracing the "rv" in the callee.
            std::vector<Instruction*> newCallsites(uloc->ctx->callSites->begin(),
                                                   uloc->ctx->callSites->end());
            newCallsites.push_back(dyn_cast<Instruction>(np));
            newCallsites.push_back(callee->getEntryBlock().getFirstNonPHIOrDbg());
            //Note that this new calling context may or may not exist in CallContext::dir
            //(e.g., the callee hasn't been analyzed due to the depth limit).
            //Since this function is only called in the bug detection phase, we can simply
            //create a new context w/o initing the pto/taint records.
            CallContext *newCtx = CallContext::getContext(&newCallsites, true);
            assert(newCtx);
            InstLoc *newLoc = InstLoc::getLoc(rv, newCtx);
            assert(newLoc);
            return is_loc_a2u(aloc,newLoc);
        } else {
            return false;
    ...
    int BBTraversalHelper::getPathsToRet(std::vector<llvm::BasicBlock*> &path,
                                std::set<std::vector<llvm::BasicBlock*>> &res) {
        if (path.empty()) {
            return -1;
        }
        //DFS for all paths to the return.
        BasicBlock *currBB = path.back();
        if (!currBB || !currBB->getTerminator()) {
            return -1;
        }
        if (currBB->getTerminator()->getNumSuccessors() == 0) {
            //The current BB is the return BB, we've found a path.
            res.insert(path);
            return 0;
        }
        //Continue the search.
        for (llvm::succ_iterator sit = llvm::succ_begin(currBB); 
            ...
            BBTraversalHelper::getPathsToRet(path,res);
            path.pop_back();
        }
        return 0;
    }
