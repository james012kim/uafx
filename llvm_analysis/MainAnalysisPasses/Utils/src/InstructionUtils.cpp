...
//#define DEBUG_GET_SINK_INSTS

    ModuleSlotTracker *mSlotTracker = nullptr;
    ...
    //hz: my feeling is that above "getCorrectInstrLocation" is unnecessarily complex and we need a better 
    //way to handle the function inline.
    //This function simplifies the implementation and returns all layers of functino inline sites
    //if any (e.g., A() inlined in B() which is inlined in C()).
    //res[0] will be the actual location of "I", res[1] is the 1-layer up inline site,
    //res[2] 2-layer up, so on and so forth.
    int InstructionUtils::getCorrectInstLoc(Instruction *I, std::vector<DILocation*> &res) {
        if (!I) {
            return 0;
        }
    ...
    std::string getFuncnameFromDLoc(DILocation *dloc) {
        if (!dloc) {
            return "";
        }
        DILocalScope *dscope = dloc->getScope();
        while (dscope) {
            if (dyn_cast<DISubprogram>(dscope)) {
                return dyn_cast<DISubprogram>(dscope)->getName().str();
            }
            dscope = dyn_cast<DILocalScope>(dscope->getScope());
        }
        return "";
    }
    ...
    int InstructionUtils::getHostFuncsFromDLoc(Instruction *I, std::vector<std::string> &res) {
        std::vector<DILocation*> dlocs;
        InstructionUtils::getCorrectInstLoc(I, dlocs);
        if (dlocs.empty()) {
            return 0;
        }
        res.clear();
        for (DILocation *dloc : dlocs) {
            std::string fname = getFuncnameFromDLoc(dloc);
            if (!fname.empty()) {
                res.push_back(fname);
            }
        }
    ...
    int InstructionUtils::getInstrLineNumber(Instruction *I) {
        ...
        InstructionUtils::getCorrectInstLoc(I, dlocs);
        if (!dlocs.empty()) {
            return getCorrectLineNumber(dlocs[0]);
        }
        return -1;
    }
    ...
    void InstructionUtils::printInst(Instruction *I, raw_ostream &ROS) {
        ...
        InstructionUtils::getCorrectInstLoc(I, dlocs);
        if (!dlocs.empty()) {
            OS << InstructionUtils::escapeJsonString(dlocs[0]->getFilename().str());
            OS << " @ " << getCorrectLineNumber(dlocs[0]);
        } else {
            OS << "N/A";
        }
        OS << "\n";
    ...
    void InstructionUtils::printInstJson(Instruction *I, raw_ostream &OS) {
        ...
        InstructionUtils::getCorrectInstLoc(I, dlocs);
        if(!dlocs.empty()) {
            for (unsigned i = 0; i < dlocs.size(); ++i) {
                O << "{\"at_line\":" << getCorrectLineNumber(dlocs[i]) << ",\"at_file\":\""
                  << InstructionUtils::escapeJsonString(dlocs[i]->getFilename().str()) << "\"}";
                if (i < dlocs.size() - 1) {
                    O << ",";
                }
            }
        ...
        if (BBNameMap.find(B) == BBNameMap.end()) {
            if (B) {
                if (B->hasName()){
    	            ...
    	            B->printAsOperand(OS, false, *mSlotTracker);
                    BBNameMap[B] = OS.str();
                }
            }else{
                BBNameMap[B] = "";
    ...
    bool InstructionUtils::getConstantValue(Constant *C, int64_t *sres, uint64_t *ures) {
        ...
        if (dyn_cast<llvm::ConstantInt>(C)) {
            ...
            return true;
                   ...
                   dyn_cast<llvm::ConstantExpr>(C)->isCast()) {
            //This covers cases like an emb "inttoptr" cast.
            Value *v = dyn_cast<llvm::ConstantExpr>(C)->getOperand(0);
            if (dyn_cast<Constant>(v)) {
                return InstructionUtils::getConstantValue(dyn_cast<Constant>(v), sres, ures);
            }
            return false;
        }
        return false;
    ...
    Value* InstructionUtils::stripAllCasts(Value* v, bool for_scalar) {
        while (v) {
            ...
            if (dyn_cast<llvm::PtrToIntOperator>(v)) {
                if (for_scalar) {
                    //This means it's not a real scalar, but just converted
                    //from a pointer, we may ignore this case if specified.
                    return nullptr;
                } else {
            ...
            //Note: some inline cast cannot be captured as operators, but
            //are implemented as constant expr, such as "inttoptr".
            if (dyn_cast<llvm::ConstantExpr>(v) &&
                dyn_cast<llvm::ConstantExpr>(v)->isCast()) {
                v = dyn_cast<llvm::ConstantExpr>(v)->getOperand(0);
                continue;
            }
    ...
    Value *InstructionUtils::stripAllSoleTrans(Value *v) {
        ...
        while (v) {
            ...
            //Stop at the call site, since the callee is a blackbox for us.
    ...
    //Self-Store: load a value from a mem location, do some sole transformations,
    //and then store back to the same mem location.
    ...
    bool InstructionUtils::same_types(Type* ty0, Type* ty1, bool wild_intp) {
        ...
        if (wild_intp) {
            ...
            if (InstructionUtils::isPrimitiveTy(ty0,8) || InstructionUtils::isPrimitiveTy(ty1,8)) {
                return (!InstructionUtils::isCompTy(ty0) && !InstructionUtils::isCompTy(ty1));
            }
        }
        ...
        Type *seq_ty0 = nullptr;
        uint64_t seq_len0 = 0;
        //https://llvm.org/doxygen/classllvm_1_1Type.html#a5e9e1c0dd93557be1b4ad72860f3cbda
        if (ty0->isStructTy()) {
            //Quick path: Compare two struct types by their names
            //(w/ numeric suffix trimmed).
            ...
            //NOTE: Not all structs are named, if the comparison involves an
            //unnamed struct, we resort to the final fallback method. 
        } else if (InstructionUtils::isSeqTy(ty0, &seq_ty0, &seq_len0)) {
            Type *seq_ty1 = nullptr;
            uint64_t seq_len1 = 0;
            InstructionUtils::isSeqTy(ty1, &seq_ty1, &seq_len1);
            assert(seq_ty1 && seq_ty0);
            //Their #elem and element type must be the same.
            return (seq_len0 == seq_len1 && InstructionUtils::same_types(seq_ty0, seq_ty1));
        }
        if (n <= 1) {
        ...
        //As the final fallback, compare the contained sub types one by one.
        for (unsigned i = 0; i < n; ++i) {
#ifdef DEBUG_TYPE_CMP
            dbgs() << i << ": " << InstructionUtils::getTypeName(ty0->getContainedType(i)) << " | " 
            << InstructionUtils::getTypeName(ty1->getContainedType(i)) << "\n";
#endif
    ...
    void getSinkInsts(Value *v, std::vector<Value*> &history, std::set<StoreInst*> &res) {
        static std::map<Value*,std::set<StoreInst*>> sink_cache;
#ifdef DEBUG_GET_SINK_INSTS
        dbgs() << "getSinkInsts(): for v: " << InstructionUtils::getValueStr(v) << "\n";
#endif
        if (!v) {
            return;
        }
        ...
        if (std::find(history.begin(), history.end(), v) != history.end()) {
            // A data loop detected, abort..
            return;
        }
        history.push_back(v);
    ...
    void getGEPDstTypeField(GEPOperator *gep, TypeField &res) {
        ...
        if (!InstructionUtils::isCompTy(baseTy)) {
            return;
        }
        Type *curTy = baseTy;
        res.ty = curTy;
        ...
        while (++i < gep->getNumOperands() && curTy) {
            ...
            res.ty = curTy;
            res.fid = fid;
            if (InstructionUtils::isCompTy(curTy) && InstructionUtils::isIndexValid(curTy,fid)) {
    ...
    std::map<Type*,std::set<long>> *InstructionUtils::getUsesInStruct(Value *v) {
        static std::map<Value*,std::map<Type*,std::set<long>>> use_cache;
        if (!v) {
            return nullptr;
        }
        if (use_cache.find(v) != use_cache.end()) {
        ...
        std::map<ConstantAggregate*,std::set<long>> *constU = 
                        InstructionUtils::getUsesInGlobalConstStruct(v);
        std::map<Type*,std::set<long>> dynU;
        //Then dynamic ones.
        //Step 1. Get the store inst(s) to store this value to somewhere.
        std::set<StoreInst*> stInsts;
        std::vector<Value*> shis;
        getSinkInsts(v,shis,stInsts);
        ...
        for (StoreInst *si : stInsts) {
            ...
            if (dyn_cast<GEPOperator>(dstp)) {
                ...
                if (InstructionUtils::isCompTy(tf.ty)) {
                    dynU[tf.ty].insert(tf.fid);
                }
            }
        }
        ...
        if (constU) {
            for (auto &e : *constU) {
                ...
                if (!constA || !InstructionUtils::isCompTy(constA->getType())) {
                    continue;
                }
                dynU[constA->getType()].insert(e.second.begin(),e.second.end());
            }
        }
    ...
    GetElementPtrInst *InstructionUtils::createSubGEP(GEPOperator* gep,unsigned i) {
        ...
        return GetElementPtrInst::Create(pointeeTy/*PointeeType*/,
                                         ptr/*Value *Ptr*/,
                                         IdxList/*ArrayRef<Value*> IdxList*/
                                         /*const Twine &NameStr=""*/ 
    ...
    *****************************************/
    ...
    std::vector<FieldDesc*> *InstructionUtils::getCompTyDesc(DataLayout *dl, Type *ty) {
        static std::map<Type*, std::vector<FieldDesc*>> descs;
        ...
        Type *seq_ty = nullptr;
        uint64_t seq_len = 0;
        if (InstructionUtils::isSeqTy(ty, &seq_ty, &seq_len)) {
            //NOTE: in the kernel we can often see the zero-length array
            //at the end of a struct (i.e. flexible length array).
            //This is normal so we shouldn't return nullptr, instead we
            //return a null content resDesc in the end of this function.
            if (seq_len > 0) {
                if (!seq_ty) {
                    return nullptr;
                }
                std::vector<FieldDesc*> *pdesc = nullptr;
                if (InstructionUtils::isCompTy(seq_ty)) {
                    pdesc = InstructionUtils::getCompTyDesc(dl,seq_ty);
                }
                //NOTE: we use alloc size here since seq_ty will be consecutively
                //stored in the array, so we consider its alloc size.
                unsigned step = dl->getTypeAllocSizeInBits(seq_ty);
                if (!pdesc) {
                    //Either non-composite element or failed to get type desc
                    //of the composite element, just treat the element as an atom.
                    for (unsigned i = 0; i < seq_len; ++i) {
                        FieldDesc *fd = new FieldDesc();
                        fd->bitoff = step * i;
                        fd->tys.push_back(seq_ty);
                        if (!i) {
                            fd->tys.push_back(ty);
                        }
                        fd->host_tys.push_back(ty);
                        fd->fid.push_back(i);
                        resDesc.push_back(fd);
                    }
                } else {
                    //For each field desc within the element type, we need to append the info
                    //of its location in the outer array.
                    for (unsigned i = 0; i < seq_len; ++i) {
            ...
            for (unsigned i = 0; i < stTy->getNumElements(); ++i) {
                ...
                if (InstructionUtils::isCompTy(ety)) {
                    ...
                    std::vector<FieldDesc*> *pdesc = InstructionUtils::getCompTyDesc(dl,ety);
                    if (pdesc) {
                        for (unsigned j = 0; j < pdesc->size(); ++j) {
                            FieldDesc *fd = new FieldDesc((*pdesc)[j]);
                            if (!i && !j) {
            ...
            dbgs() << "Cannot recognize the composite type: " << InstructionUtils::getTypeName(ty) << "\n";
            assert(false);
        }
#ifdef DEBUG_GET_TY_DESC
    ...
    bool InstructionUtils::isIndexValid(Type *ty, long fid) {
        ...
        if (fid == -1 && (ty->isArrayTy() || ty->isVectorTy())) {
            return true;
        }
    ...
    Type *InstructionUtils::getTypeAtIndex(Type *ty, long fid, int *err) {
        ...
        *err = 0;
        ...
        if (ety->isStructTy()) {
            ...
            ety = ety->getStructElementType(fid);
        } else if (InstructionUtils::isSeqTy(ety, &seq_ty)) {
            //Boundary check, note that "isIndexValid()" is aware of the "-1" index
            //for the sequential types.
            if (!InstructionUtils::isIndexValid(ety,fid)) {
                *err = 2;
                return nullptr;
            }
            ...
            //This is not a composite obj, so we don't know the
            //field type at the non-zero fid.
            *err = 3;
            return nullptr;
        }
        return ety;
    ...
    std::string InstructionUtils::getStFieldName(Module *mod, StructType *ty, unsigned fid) {
        ...
        dbgs() << "InstructionUtils::getStFieldName(): for ty: " << InstructionUtils::getTypeName(ty) << " | " << fid << "\n";
#endif
...
#ifdef DEBUG_RELATE_TYPE_NAME
            dbgs() << "InstructionUtils::getStTypeByName(): Got the type: " << InstructionUtils::getTypeName(tyMap[n]) << "\n";
#endif
            return tyMap[n];
        }
        return nullptr;
        ...
        for (i = 1; i < gep->getNumOperands(); ++i) {
            ...
            if (i > 1) {
                ...
                if (InstructionUtils::isCompTy(curTy) && InstructionUtils::isIndexValid(curTy,fid)) {
                    ...
                    std::vector<FieldDesc*> *tyd = InstructionUtils::getCompTyDesc(dl,curTy);
    ...
    std::string& InstructionUtils::getTypeName(Type *ty) {
        ...
        if (cache.find(ty) == cache.end()) {
            ...
            if (!ty) {
                cache[ty].clear();
            } else if (!InstructionUtils::isCompTy(ty)) {
            ...
            } else if (InstructionUtils::isSeqTy(ty,&seq_ty,&seq_len)) {
                std::string es = InstructionUtils::getTypeName(seq_ty);
                cache[ty] = "[" + es + "]*" + std::to_string(seq_len);
            } else {
                cache[ty] = "UNK";
            }
        }
    ...
    double InstructionUtils::getTimeDuration(std::chrono::time_point<std::chrono::system_clock> prev, raw_ostream *OS) {
        auto t = std::chrono::system_clock::now();
        std::chrono::duration<double> du = t - prev;
        double secs = du.count();
        if (OS) {
            (*OS) << secs << "s\n";
        }
        return secs;
...
out:
    ...
    int testTyCompat(Type *ty0, Type *ty1) {
        ...
        if (!InstructionUtils::isCompTy(ty0) && !InstructionUtils::isCompTy(ty1)) {
...
#ifdef DEBUG_RELATE_TYPE_NAME
            dbgs() << "InstructionUtils::inferPointeeTy(): res type: " << InstructionUtils::getTypeName(vtMap[v]) << "\n";
#endif
            return vtMap[v];
        }
...
#ifdef DEBUG_RELATE_TYPE_NAME
        dbgs() << "InstructionUtils::inferPointeeTy(): res type: " << InstructionUtils::getTypeName(hostTy) << "\n";
#endif
        vtMap[v] = hostTy;
        return hostTy;
    }
    ...
    void InstructionUtils::filterPossibleFunctionsByLoc(Instruction *inst, std::set<Function*> &targetFunctions) {
        ...
        std::vector<DILocation*> dlocs;
        InstructionUtils::getCorrectInstLoc(inst, dlocs);
        if (!dlocs.empty()) {
            std::string currFileName = dlocs[0]->getFilename().str();
            ...
            for (Function *cf : targetFunctions) {
                ...
                std::vector<DILocation*> flocs;
                InstructionUtils::getCorrectInstLoc(cf->getEntryBlock().getFirstNonPHIOrDbg(),
                                                    flocs);
                if(!flocs.empty()) {
                    currFileName = flocs[0]->getFilename().str();
    ...
    std::string InstructionUtils::isRecurTy(Type *ty) {
        static std::string rtys[] = {
            "struct.list_head", "struct.rb_node", "struct.hlist_node",
            "struct.hlist_head", "struct.rb_root", "struct.hash_cell",
        };
    ...
    std::string InstructionUtils::isRecurPtrTy(Type *ty) {
        std::string nty;
        if (!ty || !dyn_cast<PointerType>(ty)) {
            return nty;
        }
        return InstructionUtils::isRecurTy(ty->getPointerElementType());
    }
    ...
    bool InstructionUtils::seq_compatiable(StructType *ty, Type **bty, int *sz) {
        ...
        for (unsigned i = 0; i < ty->getNumElements(); ++i) {
            ...
            if (InstructionUtils::isPrimitiveTy(ty0)) {
                ++cnt;
            } else if (InstructionUtils::isSeqTy(ty0, &seq_ty, &seq_len)) {
                cnt += seq_len;
                ty0 = seq_ty;
    ...
    //Get the "->private" pointee type of the file struct as pointed to by "p".
    ...
    //NOTE: the inference is based only on the below IR sequence:
    //%0 = GEP p, 0, 16
    //%1 = CAST (i8**) %0 -> (struct.x **)
    //TODO: This is incomplete, we need to handle more possible patterns.
    void InstructionUtils::probeFilePrivTy(Value *p, std::set<Type*> &retSet) {
        if (!p) {
            return;
        }
        for (User *u : p->users()) {
    ...
    bool InstructionUtils::containTy(Type *ty, const std::string &tyName) {
        if (!InstructionUtils::isCompTy(ty)) {
            return false;
        }
        Type *seq_ty = nullptr;
        if (InstructionUtils::isSeqTy(ty, &seq_ty)) {
            return InstructionUtils::containTy(seq_ty, tyName);
        }
        if (!dyn_cast<StructType>(ty)) {
            return false;
        }
        StructType *st = dyn_cast<StructType>(ty);
        // Is it the struct itself already the specified?
        if (st->getStructName().str().find(tyName) == 0) {
            return true;
        }
        // Otherwise inspect the contained sub-types.
        for (unsigned i = 0; i < st->getNumElements(); ++i) {
            if (InstructionUtils::containTy(st->getTypeAtIndex(i), tyName)) {
                return true;
            }
        }
        return false;
    ...
    bool InstructionUtils::isTyAtIndex(Type *ty, long fid, const std::string &tyName) {
        if (!InstructionUtils::isCompTy(ty)) {
            return false;
        }
        Type *seq_ty = nullptr;
        if (InstructionUtils::isSeqTy(ty, &seq_ty)) {
            return InstructionUtils::isTyAtIndex(seq_ty, 0, tyName);
        }
        if (!dyn_cast<StructType>(ty)) {
            return false;
        }
        ...
        // Otherwise inspect the type at the specified index.
        if (fid < 0 || fid >= st->getNumElements()) {
            return false;
        }
        Type *ety = st->getTypeAtIndex(fid);
        return InstructionUtils::isTyAtIndex(ety, 0, tyName);
    }

    bool InstructionUtils::isRecurTyAtIndex(Type *ty, long fid) {
        if (!ty) {
            return false;
        }
        Type *seq_ty = nullptr;
        if (InstructionUtils::isSeqTy(ty, &seq_ty)) {
            return InstructionUtils::isRecurTyAtIndex(seq_ty, 0);
        }
        //Is the type already a recursive one?
        //Note that if so, the "fid" will be ignored.
        if (InstructionUtils::isRecurTy(ty) != "" ||
            InstructionUtils::isRecurPtrTy(ty) != "") {
            return true;
        }
        //Otherwise inspect the type at the fid.
        StructType *st = dyn_cast<StructType>(ty);
        if (!st) {
            return false;
        }
        ...
        Type *ety = st->getTypeAtIndex(fid);
        //TODO: consider the field name based heuristics, e.g., a field
        //named "next" likely indicates a linked list.
        //Heuristic: if the field is a pointer to the same host struct type,
        //it's likely a recursive strcut.
        if (ety && dyn_cast<PointerType>(ety)) {
            if (InstructionUtils::same_types(ety->getPointerElementType(),ty)) {
                return true;
            }
        }
        return InstructionUtils::isRecurTyAtIndex(ety, 0);
    }

    int InstructionUtils::getInvolvedValues(Value *v, std::set<Value *> &res,
                                            bool inc_const)
    {
        if (!v) {
            return 0;
        }
        if (inc_const || !dyn_cast<Constant>(v)) {
            res.insert(v);
        }
        if (dyn_cast<User>(v)) {
            User *u = dyn_cast<User>(v);
            for (Value *op : u->operands()) {
                if (op && (inc_const || !dyn_cast<Constant>(op))) {
                    res.insert(op);
                }
            }
        }
